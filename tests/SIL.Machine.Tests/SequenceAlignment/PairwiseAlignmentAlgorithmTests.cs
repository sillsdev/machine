...
public class PairwiseAlignmentAlgorithmTests : AlignmentAlgorithmTestsBase
{
    ...
    public void GlobalAlign()
    {
        ...
        Assert.That(alignments, Has.Length.EqualTo(1));
        ...
        Assert.That(alignments, Has.Length.EqualTo(2));
    ...
    {
        var scorer = new StringScorer();
        var paa = new PairwiseAlignmentAlgorithm<string, char>(scorer, "", "", GetChars) { Mode = AlignmentMode.Local };
        paa.Compute();
        Alignment<string, char>[] alignments = paa.GetAlignments().ToArray();
