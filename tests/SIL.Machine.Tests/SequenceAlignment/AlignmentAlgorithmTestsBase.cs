...
public abstract class AlignmentAlgorithmTestsBase
{
    ...
    {
        var sequences = new Tuple<string, AlignmentCell<char>, IEnumerable<AlignmentCell<char>>, AlignmentCell<char>>[
            alignment.GetLength(0)
        ];
        for (int i = 0; i < alignment.Length; i++)
        {
            ...
            string[] cellStrings = split[1].Trim().Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
            var cells = new AlignmentCell<char>[cellStrings.Length];
            for (int j = 0; j < cellStrings.Length; j++)
            {
                ...
                {
                    sb.Append(cellStrings[j]);
                    cells[j] = new AlignmentCell<char>(cellStrings[j]);
                }
            }
            ...
            sequences[i] = Tuple.Create(
                sb.ToString(),
                new AlignmentCell<char>(prefix),
                (IEnumerable<AlignmentCell<char>>)cells,
                new AlignmentCell<char>(suffix)
            );
        }
