...
namespace SIL.Machine.Corpora
{
    ...
    public class CorporaExtensionsTests
    {
        ...
        public void ExtractScripture()
        {
            ...
            Assert.That(corpusRef.Value, Is.EqualTo(new VerseRef("LEV 14:55", corpus.Versification)));
            ...
            Assert.That(corpusRef.HasValue, Is.True);
            Assert.That(corpusRef.Value, Is.EqualTo(new VerseRef("1CH 12:3", corpus.Versification)));
            ...
            Assert.That(corpusRef.HasValue, Is.True);
            Assert.That(corpusRef.Value, Is.EqualTo(new VerseRef("1CH 12:7", corpus.Versification)));
            ...
            Assert.That(corpusRef.HasValue, Is.True);
            Assert.That(corpusRef.Value, Is.EqualTo(new VerseRef("1CH 12:8", corpus.Versification)));
            ...
            Assert.That(corpusRef.HasValue, Is.True);
            Assert.That(corpusRef.Value, Is.EqualTo(new VerseRef("MAT 2:11", corpus.Versification)));

            (text, origRef, corpusRef) = lines[23249];
            Assert.That(text, Is.EqualTo(""));
            Assert.That(origRef, Is.EqualTo(new VerseRef("MAT 2:12", ScrVers.Original)));
            Assert.That(corpusRef.HasValue, Is.True);
            Assert.That(corpusRef.Value, Is.EqualTo(new VerseRef("MAT 2:12", corpus.Versification)));
        }
    }
}
