...
public class UsxZipTextTests
{
    ...
    public void GetRows_NonEmptyText()
    {
        using (var env = new DblBundleTestEnvironment())
        {
            ...
            Assert.That(segments[0].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 1:1", env.Corpus.Versification)));
            Assert.That(segments[0].Text, Is.EqualTo("Chapter one, verse one."));

            Assert.That(segments[1].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 1:2", env.Corpus.Versification)));
            Assert.That(segments[1].Text, Is.EqualTo("Chapter one, verse two."));

            Assert.That(segments[4].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 1:5", env.Corpus.Versification)));
            Assert.That(segments[4].Text, Is.EqualTo("Chapter one, verse five."));

            Assert.That(segments[5].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 2:1", env.Corpus.Versification)));
            Assert.That(segments[5].Text, Is.EqualTo("Chapter two, verse one."));

            Assert.That(segments[6].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 2:2", env.Corpus.Versification)));
            ...
            Assert.That(segments[7].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 2:3", env.Corpus.Versification)));
            Assert.That(segments[7].Text, Is.Empty);
            Assert.That(segments[7].IsInRange, Is.True);

            Assert.That(segments[8].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 2:4a", env.Corpus.Versification)));
            Assert.That(segments[8].Text, Is.Empty);
            Assert.That(segments[8].IsInRange, Is.True);

            Assert.That(segments[9].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 2:4b", env.Corpus.Versification)));
            Assert.That(segments[9].Text, Is.EqualTo("Chapter two, verse four."));

            Assert.That(segments[10].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 2:5", env.Corpus.Versification)));
            Assert.That(segments[10].Text, Is.EqualTo("Chapter two, verse five."));

            Assert.That(segments[11].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 2:6", env.Corpus.Versification)));
    ...
    public void GetRows_SentenceStart()
    {
        using (var env = new DblBundleTestEnvironment())
        {
            ...
            Assert.That(segments[3].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 1:4", env.Corpus.Versification)));
