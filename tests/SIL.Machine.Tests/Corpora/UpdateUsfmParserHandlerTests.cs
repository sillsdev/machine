...
";
        ...
        string targetDiffParagraph = UpdateUsfm(
            rows,
            usfm,
            textBehavior: UpdateUsfmTextBehavior.StripExisting,
            paragraphBehavior: UpdateUsfmMarkerBehavior.Strip,
...
";
        ...
        Assert.That(usfmUpdateBlockHandler.Blocks, Has.Count.EqualTo(1));
...
";
        ...
        Assert.That(usfmUpdateBlockHandler.Blocks, Has.Count.EqualTo(2));
...
";
        ...
        Assert.That(usfmUpdateBlockHandler.Blocks, Has.Count.EqualTo(4));
...
";
        ...
        Assert.That(usfmUpdateBlockHandler.Blocks, Has.Count.EqualTo(3));
        AssertUpdateBlockEquals(usfmUpdateBlockHandler.Blocks[0], ["MAT 1:0/1:p"]);
        AssertUpdateBlockEquals(
            usfmUpdateBlockHandler.Blocks[1],
            ["MAT 1:1/1:s"],
...
";
    ...
    {
        IEnumerable<ScriptureRef> parsedExtractedRefs = expectedRefs.Select(r => ScriptureRef.Parse(r));
        using (Assert.EnterMultipleScope())
        {
            Assert.That(block.Refs.SequenceEqual(parsedExtractedRefs));
            Assert.That(block.Elements, Has.Count.EqualTo(expectedElements.Length));
            foreach (
                (
            ...
            )
            {
                ...
                Assert.That(
                    string.Join(string.Empty, element.Tokens.Select(t => t.ToUsfm())),
