	...
	public class DblBundleTextTests
	{
		...
		public void Segments_SentenceStart()
		{
			using (var env = new DblBundleTestEnvironment())
			{
				...
				Assert.That(segments[38].Segment,
					Is.EqualTo("So Joseph got up , he took the child and his mother that night , and they fled to Egypt .".Split()));
				...
				Assert.That(segments[46].Segment,
					Is.EqualTo("When Joseph heard that Archaelaus now ruled in Judea district instead of his father , King Herod the Great , he was afraid to go there . Because he was warned { God warned Joseph } in a dream that it was still dangerous for them to live in Judea , he and Mary and Jesus went to Galilee District".Split()));
				...
				Assert.That(segments[47].Segment,
					Is.EqualTo("to the town called Nazareth to live there . The result was that what had been said by the ancient prophets { what the ancient prophets had said } about the Messiah , that he would be called { people would call him } a Nazareth-man , was fulfilled { came true } .".Split()));
