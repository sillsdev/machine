...
public class UsfmFileTextTests
{
    ...
    public void GetRows_NonEmptyText_AllText()
    {
        ...
        Assert.That(rows[3].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 1:0/4:s", corpus.Versification)));
        Assert.That(rows[3].Text, Is.EqualTo("Chapter One"));

        Assert.That(rows[9].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 2:0/1:s1", corpus.Versification)));
        Assert.That(rows[9].Text, Is.EqualTo("Chapter Two"));

        Assert.That(rows[13].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 2:3/1:esb/1:ms", corpus.Versification)));
        Assert.That(rows[13].Text, Is.EqualTo("This is a sidebar"));

        Assert.That(rows[14].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 2:3/1:esb/2:p", corpus.Versification)));
        Assert.That(rows[14].Text, Is.EqualTo("Here is some sidebar content."));

        Assert.That(rows[20].Ref, Is.EqualTo(ScriptureRef.Parse("MAT 2:7a/1:s", corpus.Versification)));
        Assert.That(rows[20].Text, Is.EqualTo("Section header"));
    }
