...
public class LatinWordDetokenizerTests
{
    ...
    public void Detokenize_PunctuationAtEndOfWord()
    {
        var detokenizer = new LatinWordDetokenizer();
        Assert.That(
            detokenizer.Detokenize(new[] { "This", "is", "a", "test", ",", "also", "." }),
    ...
    public void Detokenize_PunctuationAtStartOfWord()
    {
        var detokenizer = new LatinWordDetokenizer();
        Assert.That(
            detokenizer.Detokenize(new[] { "Is", "this", "a", "test", "?", "(", "yes", ")" }),
    ...
    {
        var detokenizer = new LatinWordDetokenizer();
        Assert.That(detokenizer.Detokenize(new[] { "He", "had", "$", "50", "." }), Is.EqualTo("He had $50."));
    }
    ...
    public void Detokenize_Quotes()
    {
        var detokenizer = new LatinWordDetokenizer();
        Assert.That(
            detokenizer.Detokenize(new[] { "\"", "This", "is", "a", "test", ".", "\"" }),
    ...
    public void Detokenize_Slash()
    {
        var detokenizer = new LatinWordDetokenizer();
        Assert.That(
            detokenizer.Detokenize(new[] { "This", "is", "a", "test", "/", "trial", "." }),
