...
public class ZwspWordTokenizerTests
{
    ...
    public void Tokenize_Space()
    {
        var tokenizer = new ZwspWordTokenizer();
        Assert.That(
            tokenizer.Tokenize("គែស\u200bមាង់ អី\u200bនៃ\u200bជេង\u200bនារ\u200bត៝ល់\u200bព្វាន់។"),
    ...
    public void Tokenize_Guillemet()
    {
        var tokenizer = new ZwspWordTokenizer();
        Assert.That(
            tokenizer.Tokenize("ឞ្ក្នៃ\u200bរាញា «នារ» ជេសរី"),
    ...
    public void Tokenize_Punctuation()
    {
        var tokenizer = new ZwspWordTokenizer();
        Assert.That(
            tokenizer.Tokenize("ไป\u200bไหน\u200bมา? เขา\u200bถาม\u200bผม."),
            Is.EqualTo(new[] { "ไป", "ไหน", "มา", "?", "เขา", "ถาม", "ผม", "." })
        );

        Assert.That(
            tokenizer.Tokenize("ช้าง, ม้า, วัว, กระบือ"),
    ...
    public void Tokenize_PunctuationInsideWord()
    {
        var tokenizer = new ZwspWordTokenizer();
        Assert.That(
            tokenizer.Tokenize("เริ่ม\u200bต้น\u200bที่ 7,999 บาท"),
