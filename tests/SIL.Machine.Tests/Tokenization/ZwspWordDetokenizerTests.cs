...
namespace SIL.Machine.Tokenization
{
    ...
    public class ZwspWordDetokenizerTests
    {
        ...
        public void Detokenize_Space()
        {
            var detokenizer = new ZwspWordDetokenizer();
            Assert.That(
                detokenizer.Detokenize(new[] { "គែស", "មាង់", " ", "អី", "នៃ", "ជេង", "នារ", "ត៝ល់", "ព្វាន់", "។" }),
        ...
        public void Detokenize_Guillemet()
        {
            var detokenizer = new ZwspWordDetokenizer();
            Assert.That(
                detokenizer.Detokenize(new[] { "ឞ្ក្នៃ", "រាញា", "«", "នារ", "»", "ជេសរី" }),
        ...
        public void Detokenize_Punctuation()
        {
            var detokenizer = new ZwspWordDetokenizer();
            Assert.That(
                detokenizer.Detokenize(new[] { "ไป", "ไหน", "มา", "?", "เขา", "ถาม", "ผม", "." }),
                Is.EqualTo("ไป\u200bไหน\u200bมา? เขา\u200bถาม\u200bผม.")
            );

            Assert.That(
                detokenizer.Detokenize(new[] { "ช้าง", ",", "ม้า", ",", "วัว", ",", "กระบือ" }),
        ...
        public void Detokenize_PunctuationInsideWord()
        {
            var detokenizer = new ZwspWordDetokenizer();
            Assert.That(
                detokenizer.Detokenize(new[] { "เริ่ม", "ต้น", "ที่", " ", "7,999", " ", "บาท" }),
