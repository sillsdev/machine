...
public class NmtPreprocessBuildJobTests
{
    ...
    public async Task RunAsync_FilterOutEverything()
    {
        ...
        Assert.That(await env.GetTrainCountAsync(), Is.EqualTo(0));
    }
    ...
    public async Task RunAsync_TrainOnAll()
    {
        ...
        Assert.That(await env.GetTrainCountAsync(), Is.EqualTo(5));
    }
    ...
    public async Task RunAsync_EnableKeyTerms()
    {
        ...
        Assert.That(await env.GetTrainCountAsync(), Is.EqualTo(1));
    }
    ...
    public async Task RunAsync_PretranslateChapters()
    {
        ...
        Corpus corpus1 = env.DefaultParatextCorpus with
        {
            PretranslateChapters = new Dictionary<string, IReadOnlySet<int>>
            {
                {
                    "1CH",
                    new HashSet<int> { 12 }
    ...
    public async Task RunAsync_TrainOnChapters()
    {
        ...
        Corpus corpus1 = env.DefaultParatextCorpus with
        {
            TrainOnChapters = new Dictionary<string, IReadOnlySet<int>>
            {
                {
                    "MAT",
                    new HashSet<int> { 1 }
    ...
    public async Task RunAsync_MixedSource_Paratext()
    {
        ...
        Assert.That(await env.GetTrainCountAsync(), Is.EqualTo(17));
    ...
    public async Task RunAsync_MixedSource_Text()
    {
        ...
        Assert.That(await env.GetTrainCountAsync(), Is.EqualTo(6));
        Assert.That(await env.GetPretranslateCountAsync(), Is.EqualTo(2));
    }
    ...
    private class TestEnvironment : ObjectModel.DisposableBase
    {
        ...
        public async Task<int> GetTrainCountAsync()
        {
            ...
            string srcCorpus = await srcReader.ReadToEndAsync();
            string trgCorpus = await trgReader.ReadToEndAsync();
            // Split yields one more segment that there are new lines; thus, the "- 1"
            int srcLineCount = srcCorpus.Split("\n").Length - 1;
            int trgLineCount = trgCorpus.Split("\n").Length - 1;
            Assert.That(srcLineCount, Is.EqualTo(trgLineCount));
            return srcLineCount;
        }
