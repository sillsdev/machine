...
public class NmtPreprocessBuildJobTests
{
    ...
    )
    {
        ...
        var corpus1 = new Corpus
        {
            ...
            TrainOnAll = tOAll,
            PretranslateTextIds = pTTextIds is null ? [] : pTTextIds.ToHashSet(),
            TrainOnTextIds = tOTextIds is null ? [] : tOTextIds.ToHashSet(),
            SourceFiles =
            [
                new CorpusFile
                {
        ...
        using (Stream stream = await env.SharedFileService.OpenReadAsync("builds/build1/train.src.txt"))
        {
        ...
        using (Stream stream = await env.SharedFileService.OpenReadAsync("builds/build1/pretranslate.src.json"))
        {
            ...
            Assert.That(pretranslationJsonObject!.ToList(), Has.Count.EqualTo(numEntriesWrittenToPretranslate));
        }
    }
    ...
    )
    {
        ...
        using (Stream stream = await env.SharedFileService.OpenReadAsync("builds/build1/pretranslate.src.json"))
        {
            ...
            Assert.NotNull(pretranslationJsonObject);
            Assert.That(
                pretranslationJsonObject!.ToList(),
