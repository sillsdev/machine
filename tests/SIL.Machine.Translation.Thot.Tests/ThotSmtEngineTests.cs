...
namespace SIL.Machine.Translation.Thot
{
    ...
    public class ThotSmtEngineTests
    {
        ...
        public void Translate_NBestLessThanN_Hmm()
        {
            ...
            using (ThotSmtEngine engine = smtModel.CreateEngine())
            {
                IEnumerable<TranslationResult> results = engine.Translate(
                    3,
        ...
        public void Translate_NBest_Hmm()
        {
            ...
            using (ThotSmtEngine engine = smtModel.CreateEngine())
            {
                ...
                Assert.That(
                    results.Select(tr => tr.TargetSegment),
                    Is.EqualTo(
                        new[] { "hablé until five o ' clock .".Split(), "hablé until five o ' clock for".Split() }
                    )
                );
            }
        }
        ...
        public void GetBestPhraseAlignment_SegmentPair_Hmm()
        {
            ...
            using (ThotSmtEngine engine = smtModel.CreateEngine())
            {
                TranslationResult result = engine.GetBestPhraseAlignment(
