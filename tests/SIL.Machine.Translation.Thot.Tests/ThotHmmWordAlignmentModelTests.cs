...
{
    private static string DirectModelPath => Path.Combine(TestHelpers.ToyCorpusHmmFolderName, "tm", "src_trg_invswm");
    private static string InverseModelPath => Path.Combine(TestHelpers.ToyCorpusHmmFolderName, "tm", "src_trg_swm");
    ...
    public void GetBestAlignedWordPairs()
    {
        ...
        Assert.That(pairs, Has.Length.EqualTo(8));
    ...
    public void ComputeAlignedWordPairScores()
    {
        ...
        Assert.That(pairs, Has.Length.EqualTo(11));
    ...
    {
        using var model = new ThotHmmWordAlignmentModel(DirectModelPath);
        Assert.That(model.SourceWords, Has.Count.EqualTo(513));
    }
    ...
    {
        using var model = new ThotHmmWordAlignmentModel(DirectModelPath);
        Assert.That(model.TargetWords, Has.Count.EqualTo(363));
    }
    ...
    public void GetTranslationTable_SymmetrizedNoThreshold()
    {
        ...
        Assert.That(table, Has.Count.EqualTo(513));
        Assert.That(table["es"], Has.Count.EqualTo(23));
    }
    ...
    public void GetTranslationTable_SymmetrizedThreshold()
    {
        ...
        Assert.That(table, Has.Count.EqualTo(513));
        Assert.That(table["es"], Has.Count.EqualTo(9));
    }
    ...
    public void ComputeAlignedWordPairScores_Symmetrized()
    {
        ...
        AlignedWordPair[] pairs = waMatrix.ToAlignedWordPairs(includeNull: true).ToArray();
        model.ComputeAlignedWordPairScores(sourceSegment, targetSegment, pairs);
        Assert.That(pairs, Has.Length.EqualTo(10));
