...
public sealed class CompilationGraphHashingTests
{
    ...
    public void TextEolAndPhysicalRootRelocationDoNotChangeHashes()
    {
        ...
        Assert.Multiple(() =>
        {
            Assert.That(LogicalPathTokens.FromAbsolute(
                @"C:\checkout\src\C.cs",
    ...
    public void AncestorEditorConfigFilesAreAdmittedWithALocationIndependentLogicalIdentity()
    {
        var roots = new LogicalPathRoots(
            @"C:\outer\machine\worktrees\squashed", @"C:\sdk", @"C:\nuget", @"C:\generated");

        string logical = LogicalPathTokens.FromAbsoluteAdmittingAncestorEditorConfig(
            @"C:\outer\machine\.editorconfig", roots);
    ...
    public void FromAbsoluteAdmittingAncestorEditorConfigStillFailsClosedOutsideTheEditorConfigCase()
    {
        ...
        Assert.That(
            () => LogicalPathTokens.FromAbsoluteAdmittingAncestorEditorConfig(
                @"C:\outer\machine\Directory.Build.props", roots),
    ...
    public void CompilerOrderIsSemanticButSetOrderIsNot()
    {
        ...
        Assert.That(CompilationGraphHashing.Compute(reorderedArguments).GraphInputHash,
            Is.Not.EqualTo(CompilationGraphHashing.Compute(baseline).GraphInputHash));
        Assert.That(CompilationGraphHashing.Compute(reorderedSet),
    ...
    public void LogicalPathsRejectUnknownRootsAndCaseCollisions()
    {
        ...
        Assert.That(() => LogicalPathTokens.FromAbsolute(@"C:\elsewhere\file.cs", roots),
            Throws.TypeOf<InvalidDataException>());
        Assert.That(() => LogicalPathTokens.ValidateUnique(new[] { "repo:/src/C.cs", "repo:/src/c.cs" }),
            Throws.TypeOf<InvalidDataException>());
    }
    ...
    public void ExactLogicalFileReuseAcrossProfilesIsAllowedButCaseVariantsAreRejected()
    {
        ...
        Assert.DoesNotThrow(() => new CompilationGraphHashInputs(
        ...
        Assert.That(() => new CompilationGraphHashInputs(
            "hc-compilation-graph/v1",
            new[] { new ProjectHashInput("hc", "repo:/src/hc.csproj", "netstandard2.0") },
            new[]
            {
            ...
            Toolchain(),
            File("repo:/eng/HcSemanticCompilerInputs.targets", "<Project />\n", GraphHashFileKind.Text),
            new OptionalGraphHashFile(false, null)), Throws.TypeOf<InvalidDataException>());
    }
    ...
    {
        GraphHashFile source = File("repo:/src/C.cs", "class C { }\n", GraphHashFileKind.Text);
        Assert.That(() => Node("base", source,
            arguments: new[] { new OrderedHashValue(0, "a"), new OrderedHashValue(0, "b") }),
            Throws.TypeOf<InvalidDataException>());
        Assert.That(() => Node("base", source,
            arguments: new[] { new OrderedHashValue(0, "a"), new OrderedHashValue(2, "b") }),
            Throws.TypeOf<InvalidDataException>());
        Assert.That(() => new NodeHashInput(
            ...
            Array.Empty<GraphHashFile>(),
            Array.Empty<GraphHashFile>(),
            Array.Empty<GraphHashFile>()), Throws.TypeOf<InvalidDataException>());
    }

    [Test]
    public void OverlappingPhysicalRootsAreRejectedAsAmbiguous()
    {
        Assert.That(() => new LogicalPathRoots(
    ...
    {
        var roots = new LogicalPathRoots(@"C:\repo", @"C:\sdk", @"C:\nuget", @"C:\generated");
        Assert.That(() => LogicalPathTokens.FromAbsolute(@"C:\repo\sub\..\C.cs", roots),
            Throws.TypeOf<InvalidDataException>());

        GraphHashFile windowsJson = File("repo:/obj/project.assets.json", "{\"text\":\"a\\r\\nb\"}", GraphHashFileKind.Json);
        GraphHashFile linuxJson = File("repo:/obj/project.assets.json", "{\"text\":\"a\\nb\"}", GraphHashFileKind.Json);
        Assert.That(CompilationGraphHashing.Compute(InputsWithAsset(windowsJson)),
    ...
    {
        Assert.That(() => new ProjectHashInput("hc", @"C:\repo\src\hc.csproj", "netstandard2.0"),
            Throws.TypeOf<ArgumentException>());
        Assert.That(() => new ProjectHashInput("hc", "other:/src/hc.csproj", "netstandard2.0"),
            Throws.TypeOf<ArgumentException>());
    ...
    public void FilesMapCorrectlyWhenAnAdmittedRootIsAFileSystemRoot()
    {
        ...
        Assert.That(LogicalPathTokens.FromAbsolute(
            "/repo/file.cs",
            new LogicalPathRoots("/", "/sdk", "/nuget", "/generated")),
            Is.EqualTo("repo:/repo/file.cs"));
    }
    ...
    public void HashModelsRejectNullElementsDuplicateIdentitiesAndAmbiguousNodeSegments()
    {
        ...
        Assert.That(() => new ToolchainHashInput("sdk", "msbuild", "roslyn", "compiler", "loader",
            new GraphHashFile[] { null! }), Throws.ArgumentException);
        ...
        Assert.That(() => new CompilationGraphHashInputs(
            ...
            baseline.LockFile), Throws.TypeOf<InvalidDataException>());
        Assert.That(() => new CompilationGraphHashInputs(
            baseline.SchemaVersion,
            baseline.Projects,
            baseline.Profiles,
            ...
            baseline.CaptureTarget,
            baseline.LockFile), Throws.TypeOf<InvalidDataException>());
        Assert.That(() => new RepositoryGraphNodeKey("hc/other", "netstandard2.0", "base"),
    ...
    {
        Assert.That(() => CanonicalJson.NormalizeJson(Encoding.UTF8.GetBytes("{\"a\":1,\"a\":2}")),
            Throws.TypeOf<InvalidDataException>());
        Assert.That(CanonicalJson.NormalizeJson(Encoding.UTF8.GetBytes("{\"n\":1}")),
    ...
    {
        Assert.That(() => new GraphHashFile(null!, ImmutableArray<byte>.Empty, GraphHashFileKind.Binary),
            Throws.ArgumentException);
        Assert.That(() => new ProjectHashInput("hc", null!, "netstandard2.0"),
            Throws.ArgumentException);
        Assert.That(() => new LogicalPathRoots("/", "/", "/nuget", "/generated"),
            Throws.TypeOf<InvalidDataException>());
        Assert.That(() => new LogicalPathRoots(@"C:\", @"c:\", @"D:\nuget", @"D:\generated"),
            Throws.TypeOf<InvalidDataException>());
    }
        ...
        IReadOnlyList<string>? profileSymbols = null)
    {
        ...
        return new CompilationGraphHashInputs(
            ...
            new[]
            {
                Node("base", actualSource, arguments)
            },
        ...
        new(
            ...
            new[]
            {
                new NodeHashInput(
                    ...
                    new Dictionary<string, string>(),
                    Array.Empty<OrderedHashValue>(),
                    new[] { new OrderedGraphHashFile(0, File("repo:/src/C.cs", "class C { }\n", GraphHashFileKind.Text)) },
                    Array.Empty<ReferenceHashInput>(),
                    Array.Empty<ProjectReferenceHashInput>(),
                    Array.Empty<AnalyzerHashInput>(),
                    Array.Empty<GraphHashFile>(),
