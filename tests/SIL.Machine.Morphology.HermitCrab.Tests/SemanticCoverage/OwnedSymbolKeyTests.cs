...
public sealed class OwnedSymbolKeyTests
{
    ...
    public async Task LiveBridgeRekeysRetargetedSymbolsToProfileLocalOwnedDefinitions()
    {
        ...
        INamedTypeSymbol toolDefinition = Type(graph, "hc-tool", "base", "SIL.Machine.Morphology.HermitCrab.SignatureFormat");
        INamedTypeSymbol toolFromConformance = Type(graph, "hc-conformance", "base", "SIL.Machine.Morphology.HermitCrab.SignatureFormat");
        OwnedSymbolKey toolKey = bridge.KeyFor("base", toolDefinition);

        INamedTypeSymbol conformanceDefinition = Type(graph, "hc-conformance", "base", "SIL.Machine.Morphology.HermitCrab.Conformance.Fixture");
        ...
        LocalFunctionStatementSyntax localSyntax = morpherTree.GetRoot().DescendantNodes()
            .OfType<LocalFunctionStatementSyntax>().Single(node => node.Identifier.ValueText == "GenerateSynthesis");
        IMethodSymbol local = morpherModel.GetDeclaredSymbol(localSyntax)!;
        OwnedSymbolKey localKey = bridge.KeyFor("base", local);
        IMethodSymbol[] lambdas = morpherTree.GetRoot().DescendantNodes().OfType<AnonymousFunctionExpressionSyntax>()
        ...
        IFieldSymbol analysisRuleField = Type(graph, "hc", "base", "SIL.Machine.Morphology.HermitCrab.Morpher")
            .GetMembers("_analysisRule").OfType<IFieldSymbol>().Single();
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(SymbolEqualityComparer.Default.Equals(bridge.Resolve("base", machineKey), machineDefinition), Is.True);
            Assert.That(bridge.KeyFor("base", toolFromConformance), Is.EqualTo(toolKey));
            Assert.That(SymbolEqualityComparer.Default.Equals(bridge.Resolve("base", toolKey), toolDefinition), Is.True);
            Assert.That(SymbolEqualityComparer.Default.Equals(bridge.Resolve("base", conformanceKey), conformanceDefinition), Is.True);
            Assert.That(SymbolEqualityComparer.Default.Equals(bridge.Resolve("base", idGetterKey), idGetter), Is.True);
            Assert.That(idGetterKey.Value, Does.EndWith("/SIL.Machine.Morphology.HermitCrab.Conformance.Fixture.Id/get"));
            ...
            Assert.That(resolvedConstructedRule.ContainingAssembly,
                Is.SameAs(graph.Nodes.Values.Single(item => item.Key.ProjectId == "machine" && item.Key.ProfileId == "base")
    ...
    public void CanonicalKeysCoverEveryMemberShapeWithoutCollision()
    {
        ...
        ISymbol namespaceSymbol = compilation.GetSemanticModel(tree)
            .GetDeclaredSymbol(tree.GetRoot().DescendantNodes().OfType<FileScopedNamespaceDeclarationSyntax>().Single())!;
        ...
        IMethodSymbol addition = type.GetMembers().OfType<IMethodSymbol>().Single(item => item.MethodKind == MethodKind.UserDefinedOperator);
        IMethodSymbol[] conversions = type.GetMembers().OfType<IMethodSymbol>()
            .Where(item => item.MethodKind == MethodKind.Conversion).ToArray();
        ...
        INamedTypeSymbol innerOfIntString = outerOfInt.GetTypeMembers("Inner").Single()
            .Construct(compilation.GetSpecialType(SpecialType.System_String));
        INamedTypeSymbol innerOfLongString = outerOfLong.GetTypeMembers("Inner").Single()
            .Construct(compilation.GetSpecialType(SpecialType.System_String));
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(OwnedSymbolKey.Create("fixture", method).Value,
                Does.EndWith("/Fixture.Outer`1.Method`1(!0[],ref !!0,out System.Int32,in System.String,params System.Object[])"));
            Assert.That(OwnedSymbolKey.Create("fixture", constructor).Value,
                Does.EndWith("/Fixture.Outer`1..ctor(ref System.Int32,out System.String)"));
            Assert.That(OwnedSymbolKey.Create("fixture", indexer).Value,
            ...
            Assert.That(OwnedSymbolKey.Create("fixture", conversions[0]), Is.Not.EqualTo(OwnedSymbolKey.Create("fixture", conversions[1])));
            Assert.That(OwnedSymbolKey.Create("fixture", innerOfIntString), Is.Not.EqualTo(OwnedSymbolKey.Create("fixture", innerOfLongString)));
            Assert.That(OwnedSymbolKey.Create("fixture", innerDefinition).Value, Does.EndWith("/Fixture.Outer`1.Inner`1"));
