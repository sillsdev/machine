...
public sealed class DotnetMsbuildCompilationGraphLoaderTests
{
    ...
    public void RepositoryPinsTheCompilerSdkAndRoslynPackage()
    {
        ...
        {
            Assert.That(
                global.RootElement.GetProperty("sdk").GetProperty("version").GetString(),
                Is.EqualTo("10.0.100")
            );
            Assert.That(
                global.RootElement.GetProperty("sdk").GetProperty("rollForward").GetString(),
            ...
            Assert.That(
                references,
                Does.Contain("$(MSBuildToolsPath)/Roslyn/bincore/Microsoft.CodeAnalysis.CSharp.dll")
            );
        });
    }
    ...
    public void CaptureTargetDeclaresTheNativeProtocolWithoutSerializationTasks()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(
                target,
                Does.Contain(
                    "ResolveReferences;ResolveKeySource;SetWin32ManifestProperties;FindReferenceAssembliesForReferences;BeforeCompile;CoreCompile"
                )
            );
    ...
    public void ProtocolParserAcceptsTheNativeEnvelopeAndPreservesItemOrder()
    {
        ...
        Assert.That(
            captured.Items["Compile"].Select(item => item.Identity),
            Is.EqualTo(new[] { "repo:/b.cs", "repo:/a.cs" })
        );
    ...
    {
        Assert.That(
            () => MsBuildCaptureProtocol.Parse(Encoding.UTF8.GetBytes(json)),
    ...
    {
        string duplicate =
            "{\"Properties\":{\"PanGlossCompilerInputProtocol\":\"hc-semantic-msbuild/v1\",\"PanGlossCompilerInputProtocol\":\"hc-semantic-msbuild/v1\"},\"Items\":{}}";
        string nullIdentity = CompleteEnvelope("[{\"Identity\":null}]");

        Assert.Multiple(() =>
        {
            Assert.That(
                () => MsBuildCaptureProtocol.Parse(Encoding.UTF8.GetBytes(duplicate)),
                Throws.InstanceOf<InvalidDataException>()
            );
            Assert.That(
                () => MsBuildCaptureProtocol.Parse(Encoding.UTF8.GetBytes(nullIdentity)),
                Throws.InstanceOf<InvalidDataException>()
            );
        });
    }
    ...
    {
        Assert.That(
            () => MsBuildCaptureProtocol.Parse(new byte[] { 0xFF, 0xFE }),
            Throws.InstanceOf<InvalidDataException>()
        );
    }
    ...
    public async Task LoaderUsesBoundedShellFreeQueriesAndBuildsTheSixteenNodeMatrix()
    {
        ...
        Assert.That(
            ...
            "Four base-property evaluations plus sixteen profile queries are required."
        );
        Assert.Multiple(() =>
        {
            ...
            Assert.That(
                runner
                    .Calls.Where(call => call.StartInfo.ArgumentList.Contains("/t:_PanGlossCaptureCompilerInputs"))
                    .All(call =>
                        ...
                        && call.StartInfo.ArgumentList.Any(argument =>
                            argument.StartsWith("/p:ProjectAssetsFile=", StringComparison.Ordinal)
                        )
                    ),
                Is.True
            );
            Assert.That(
                runner
                    ...
                    .All(call =>
                        call.StartInfo.ArgumentList.Any(argument =>
                            argument.StartsWith("/p:IntermediateOutputPath=", StringComparison.Ordinal)
                        )
                    ),
                Is.True
            );
            Assert.That(
                runner
                    .Calls.SelectMany(call => call.StartInfo.ArgumentList)
                    .Any(argument => argument == "/p:DefineConstants=OUTPUT_ANALYSES%3BSINGLE_THREADED%3BTRACE"),
                Is.True
            );
        });
    }
                        ...
                        new NodeHashInput(
                            ...
                            new[]
                            {
                                new OrderedGraphHashFile(
                                    0,
                                    HashFile(
                                        $"generated:/{node.ProjectId}/{node.Profile.Id}/source.cs",
                                        $"// {ordinal}"
                                    )
                                ),
                            },
            ...
            (_, _) =>
                new ProcessCapture(
                    ...
                    Encoding.UTF8.GetBytes(
                        "{\"Properties\":{\"DefineConstants\":\"TRACE\",\"TargetFramework\":\"netstandard2.0\"}}"
                    ),
                    standardError
                )
        );
    ...
    {
        var oversized = new RecordingMsBuildRunner(
            (_, _) =>
                new ProcessCapture(0, new byte[RepositoryCompilationGraphLoader.MaximumStandardOutputBytes + 1], "")
        );
        var mismatched = new RecordingMsBuildRunner(
            (start, call) =>
            {
                if (call < 4)
                {
                    return new ProcessCapture(
                        0,
                        Encoding.UTF8.GetBytes(
                            $"{{\"Properties\":{{\"DefineConstants\":\"TRACE\",\"TargetFramework\":\"{ArgumentValue(start, "/p:TargetFramework=")}\"}}}}"
                        ),
                        ""
                    );
                }
                ...
                return new ProcessCapture(
                    0,
                    Encoding.UTF8.GetBytes(
                        CompleteEnvelope("[]", target)
                            .Replace("hc-semantic-msbuild/v1", "hc-semantic-msbuild/v2", StringComparison.Ordinal)
                    ),
                    ""
                );
            }
        ...
        Assert.Multiple(() =>
        {
            Assert.That(
                async () =>
                    await new RepositoryCompilationGraphLoader(oversized).LoadAsync(
            ...
            Assert.That(
                async () =>
                    await new RepositoryCompilationGraphLoader(mismatched).LoadAsync(
                        new RepositoryRoot(RepositoryRoot()),
                        CancellationToken.None
                    ),
                Throws.TypeOf<InvalidDataException>()
    ...
    public void LoaderPropagatesTimeoutAndTerminationFailures()
    {
        ...
        var termination = new RecordingMsBuildRunner(
            (_, _) => throw new InvalidDataException("termination unconfirmed")
        );
        ...
        Assert.Multiple(() =>
        {
            Assert.That(
                async () =>
                    await new RepositoryCompilationGraphLoader(timeout).LoadAsync(
                        new RepositoryRoot(RepositoryRoot()),
                        CancellationToken.None
                    ),
                Throws.TypeOf<InvalidDataException>()
            ...
            Assert.That(
                async () =>
                    await new RepositoryCompilationGraphLoader(termination).LoadAsync(
                        new RepositoryRoot(RepositoryRoot()),
                        CancellationToken.None
                    ),
                Throws.TypeOf<InvalidDataException>()
    ...
    public void LoaderFailsBeforeProcessExecutionWhenRestoredAssetsAreMissing()
    {
        ...
        try
        {
            ...
            Assert.That(
                async () =>
                    await new RepositoryCompilationGraphLoader(runner).LoadAsync(
                        new RepositoryRoot(scratch),
                        CancellationToken.None
                    ),
    ...
    public void ProcessRunnerEnforcesTheOutputLimitWithoutUsingAShell()
    {
        ...
        Assert.That(
            async () =>
                await new MsBuildProcessRunner().RunAsync(
                    start,
    ...
    public async Task LiveLoaderCapturesEveryRestoredNodeWithoutRepositoryWrites(CancellationToken cancellationToken)
    {
        ...
        ).LoadAsync(new RepositoryRoot(root), cancellationToken);

        CapturedCompilerInputs firstCapture = graph
            .Captures.OrderBy(pair => pair.Key.ProjectId, StringComparer.Ordinal)
            .First()
            .Value;
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(
                graph.CompilerInputs.All(pair => pair.Value.Sources.All(source => source.Content.Length > 0)),
                Is.True
            );
            Assert.That(graph.CompilerInputs.All(pair => pair.Value.AnalyzerConfigs.Count > 0), Is.True);
            Assert.That(
                graph.CompilerInputs.All(pair => pair.Value.AnalyzerConfigs.All(config => config.Content.Length > 0)),
                Is.True
            );
            Assert.That(
                graph.CompilerInputs.All(pair => pair.Value.AnalyzerConfigs.Any(config => !File.Exists(config.Path))),
                Is.True,
                "Each node must retain its private generated analyzer configuration after capture cleanup."
            );
            Assert.That(
                graph
                    .CompilerInputs.Where(pair => pair.Key.TargetFramework == "net10.0")
                    .All(pair =>
            ...
            Assert.That(
                graph.Captures.All(pair =>
                    pair.Value.Properties["PanGlossCompilerInputProtocol"] == MsBuildCaptureProtocol.Version
                ),
                Is.True
            );
            Assert.That(
                graph.Captures.All(pair => pair.Value.Properties["TargetFramework"] == pair.Key.TargetFramework),
                Is.True
            );
            ...
            Assert.That(
                graph.HashInputs.Nodes.All(node => node.Assets.Length >= 3),
                Is.True,
            ...
            Assert.That(
                graph
                    .HashInputs.Nodes.Where(node => node.Key.ProjectId != "machine")
                    .All(node => node.ProjectReferences.Length > 0),
                Is.True
            );
            ...
            Assert.That(
                new[] { graph.Hashes.GraphInputHash, graph.Hashes.ToolchainHash, graph.Hashes.GraphHash }.All(hash =>
                    hash.Length == 64 && hash.All(character => char.IsAsciiHexDigitLower(character))
                ),
                Is.True
            );
    ...
    public void CommandLineParserPreservesRoslynOptionsReferencesAndAuxiliaryInputs()
    {
        ...
        var args = new[]
        {
            $"/define:TRACE;BASE;SINGLE_THREADED;OUTPUT_ANALYSES",
            "/langversion:preview",
            "/nullable:enable",
            "/unsafe+",
            "/checked+",
        ...
        {
            Assert.That(
                ((CSharpParseOptions)model.Arguments.ParseOptions).LanguageVersion,
                Is.EqualTo(LanguageVersion.Preview)
            );
            Assert.That(
                ((CSharpCompilationOptions)model.Arguments.CompilationOptions).NullableContextOptions,
                Is.EqualTo(NullableContextOptions.Enable)
            );
            ...
            Assert.That(
                model
                    .Arguments.MetadataReferences.Single(item => item.Properties.EmbedInteropTypes)
            ...
            Assert.That(
                model.Arguments.AdditionalFiles.Select(file => file.Path),
                Is.EqualTo(new[] { additionalFile })
            );
            ...
            Assert.That(
                ((CSharpParseOptions)model.Arguments.ParseOptions).PreprocessorSymbolNames,
        ...
        CompilerInputException exception = Assert.Throws<CompilerInputException>(() =>
            CSharpCommandLineInputParser.Parse(
                new[]
                {
                    "/define:TRACE",
                    "/out:test.dll",
                    Path.Combine(RepositoryRoot(), "src", "SIL.Machine", "Utils", "StringExtensions.cs"),
                },
                RepositoryRoot(),
                new[] { "OUTPUT_ANALYSES" }
            )
    ...
    public void SourceClassifierAdmitsOnlyOwnedAndKnownSdkGeneratedSupport()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(
                netstandardClassifier.Classify(netstandardAttributes),
                Is.EqualTo(CompilerSourceKind.GeneratedSupport)
            );
            Assert.That(
                () => classifier.Classify(Path.Combine(intermediate, "Evil.AssemblyInfo.cs")),
                Throws.TypeOf<CompilerInputException>().With.Property("Code").EqualTo("unsupported-compiler-source")
            );
            Assert.That(
                () => classifier.Classify(Path.Combine(intermediate, ".Spoof,Version=v999.AssemblyAttributes.cs")),
                Throws.TypeOf<CompilerInputException>().With.Property("Code").EqualTo("unsupported-compiler-source")
            );
            Assert.That(
                () => classifier.Classify(Path.Combine(intermediate, "Custom.Generated.cs")),
                Throws.TypeOf<CompilerInputException>().With.Property("Code").EqualTo("unsupported-compiler-source")
            );
            Assert.That(
                () =>
                    classifier.Classify(
                        Path.Combine(project, "Generated.cs"),
                        new Dictionary<string, string> { ["AutoGen"] = "true" }
                    ),
                Throws.TypeOf<CompilerInputException>().With.Property("Code").EqualTo("unsupported-compiler-source")
            );
            Assert.That(
                () => classifier.Classify(Path.Combine(project, "obj", "Generated.cs")),
                Throws.TypeOf<CompilerInputException>().With.Property("Code").EqualTo("unsupported-compiler-source")
            );
            Assert.That(
                () => classifier.Classify(Path.Combine(project, "GlobalUsings.g.cs")),
                Throws
                    .TypeOf<CompilerInputException>()
                    .With.Property("Code")
                    .EqualTo("unsupported-implicit-global-using")
            );
            Assert.That(
                () => classifier.Classify(Path.Combine(root, "outside.cs")),
                Throws.TypeOf<CompilerInputException>().With.Property("Code").EqualTo("unsupported-compiler-source")
            );
        });
    }
    ...
    public void AnalyzerInspectorRecordsProvenanceAndRejectsUnownedSourceGenerators()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                Assert.That(
                    ...
                    Throws
                        .TypeOf<CompilerInputException>()
                        .With.Property("Code")
                        .EqualTo("unsupported-source-generator")
                );
            }
            finally
            {
            ...
            try
            {
                ...
                AnalyzerMetadataInspection admitted = AnalyzerMetadataInspector.Inspect(
                    admittedPath,
                    new[] { admittedDirectory }
                );
            ...
            Assert.That(
                () =>
                    CSharpCommandLineInputParser.Parse(
                        new[]
                        {
                            $"/analyzer:{inheritedGeneratorPath}",
                            "/out:test.dll",
                            Path.Combine(RepositoryRoot(), "src", "SIL.Machine", "Utils", "StringExtensions.cs"),
                        },
                        RepositoryRoot()
        ...
        Assert.That(
            () => AnalyzerMetadataInspector.Inspect(Path.Combine(Path.GetTempPath(), Guid.NewGuid() + ".dll")),
    ...
    public void CompilerIdentityMismatchIsReportedBeforeParsing()
    {
        ...
        CompilerInputException exception = Assert.Throws<CompilerInputException>(() =>
            CSharpCommandLineInputParser.Parse(
                ...
                RepositoryRoot(),
                Array.Empty<string>(),
                new CompilerToolchainIdentity(other)
            )
        )!;
    ...
    public void CompilerIdentityRequiresAnAbsoluteCompilerDirectory()
    {
        CompilerInputException exception = Assert.Throws<CompilerInputException>(() =>
            CSharpCommandLineInputParser.Parse(
                Array.Empty<string>(),
                RepositoryRoot(),
                queriedToolchain: new CompilerToolchainIdentity("relative")
            )
        )!;
    ...
    private sealed class RecordingMsBuildRunner : IMsBuildProcessRunner
    {
        ...
        )
        {
            ...
            if (
                startInfo.ArgumentList.Any(argument =>
                    argument.StartsWith("-getProperty:DefineConstants", StringComparison.Ordinal)
                )
            )
            {
                ...
                return ValueTask.FromResult(
                    new ProcessCapture(
                        0,
                        Encoding.UTF8.GetBytes(
                            $"{{\"Properties\":{{\"DefineConstants\":\"TRACE\",\"TargetFramework\":\"{evaluatedTarget}\"}}}}"
                        ),
                        ""
                    )
                );
            ...
            string projectFile = startInfo.ArgumentList.Single(argument =>
            ...
            string commandLineItems = JsonSerializer.Serialize(
                new[]
                {
                    new
                    {
                        Identity = "/define:"
                            + ArgumentValue(startInfo, "/p:DefineConstants=")
            ...
            string generatedAssemblyInfo = Path.Combine(
                intermediate,
                $"{Path.GetFileNameWithoutExtension(projectFile)}.AssemblyInfo.cs"
            );
    ...
    private static IReadOnlyDictionary<string, FileStamp> RepositoryFileStamps(string root)
    {
        ...
        return projectRoots
            .SelectMany(relative =>
                Directory.EnumerateFiles(Path.Combine(root, relative), "*", SearchOption.AllDirectories)
            )
            .ToDictionary(
    ...
    private static string ArgumentValue(ProcessStartInfo startInfo, string prefix) =>
        startInfo.ArgumentList.Single(argument => argument.StartsWith(prefix, StringComparison.Ordinal))[
            prefix.Length..
        ];
        ...
        return $$"""
            {
              "Properties": {
                ...
                "GeneratedAssemblyInfoFile": {{JsonSerializer.Serialize(
                generatedAssemblyInfoFile ?? "tmp:/Project.AssemblyInfo.cs"
            )}},
                "TargetFrameworkMonikerAssemblyAttributesPath": {{JsonSerializer.Serialize(
                targetFrameworkAttributesPath ?? "tmp:/.NETStandard,Version=v2.0.AssemblyAttributes.cs"
            )}}
              },
              "Items": {
