...
public sealed class RepositoryCompilationGraphTests
{
    ...
    public void FixedGraphHasTheExactFourProjectsFourProfilesAndSixteenNodes()
    {
        ...
        Assert.That(graph.Projects.Select(project => project.Id), Is.EqualTo(new[]
        {
            "machine", "hc", "hc-tool", "hc-conformance",
        }));
        Assert.That(graph.Projects.Select(project => project.RelativePath), Is.EqualTo(new[]
        {
        ...
        Assert.That(graph.Projects.Select(project => project.TargetFramework), Is.EqualTo(new[]
        {
            "netstandard2.0", "netstandard2.0", "net10.0", "net10.0",
        }));
        Assert.That(graph.Projects.Select(project => project.DirectOwnedReferences), Is.EqualTo(new[]
        {
            Array.Empty<string>(),
            new[] { "machine" },
            new[] { "hc" },
        ...
        Assert.That(graph.Profiles.Select(profile => profile.Id), Is.EqualTo(new[]
        {
            "base", "single-threaded", "output-analyses", "combined",
        }));
        Assert.That(graph.Profiles.Select(profile => profile.AdditionalSymbols), Is.EqualTo(new[]
        {
            Array.Empty<string>(),
            new[] { "SINGLE_THREADED" },
            new[] { "OUTPUT_ANALYSES" },
        ...
        Assert.That(graph.Profiles.Single(profile => profile.Id == "combined").AdditionalSymbols,
            Is.EqualTo(new[] { "SINGLE_THREADED", "OUTPUT_ANALYSES" }));
        Assert.That(graph.Nodes, Has.Count.EqualTo(16));
        Assert.That(graph.Nodes.Select(node => node.Key), Is.EqualTo(
        ...
        Assert.That(graph.ProjectEdges, Is.EqualTo(new[]
        {
    ...
    public void MissingAndExtraNodesAreRejected()
    {
        ...
        Assert.That(() => RepositoryCompilationGraph.Create(missing, fixedGraph.ProjectEdges),
            Throws.TypeOf<InvalidDataException>());

        List<RepositoryGraphNode> extra = fixedGraph.Nodes.ToList();
        extra.Add(new RepositoryGraphNode("machine", "src/SIL.Machine/SIL.Machine.csproj", "netstandard2.0",
            fixedGraph.Profiles[0]));
        Assert.That(() => RepositoryCompilationGraph.Create(extra, fixedGraph.ProjectEdges),
            Throws.TypeOf<InvalidDataException>());
    }
    ...
    public void OutsideProjectAndCyclicEdgesAreRejected()
    {
        ...
        outside[0] = new RepositoryGraphNode("machine", "../other/SIL.Machine.csproj", "netstandard2.0",
            outside[0].Profile);
        Assert.That(() => RepositoryCompilationGraph.Create(outside, fixedGraph.ProjectEdges),
            Throws.TypeOf<InvalidDataException>());
        ...
        Assert.That(() => RepositoryCompilationGraph.Create(fixedGraph.Nodes, cyclic),
            Throws.TypeOf<InvalidDataException>());
    }
    ...
    public void UnknownAndAmbiguousEdgesAreRejected()
    {
        ...
        Assert.That(() => RepositoryCompilationGraph.Create(fixedGraph.Nodes, outside),
            Throws.TypeOf<InvalidDataException>());
    ...
    public void IndependentlyConstructedEquivalentProfileIsAcceptedAsTheSameClosedValue()
    {
        ...
        nodes[0] = new RepositoryGraphNode(nodes[0].ProjectId, nodes[0].ProjectPath,
            nodes[0].TargetFramework, equivalent);
    ...
    public void UnknownProfileIdAndKnownProfileWithWrongSymbolsAreRejected()
    {
        ...
        List<RepositoryGraphNode> nodes = fixedGraph.Nodes.ToList();
        nodes[0] = new RepositoryGraphNode(nodes[0].ProjectId, nodes[0].ProjectPath,
            nodes[0].TargetFramework, new BuildProfile("fifth-profile", Array.Empty<string>()));
        Assert.That(() => RepositoryCompilationGraph.Create(nodes, fixedGraph.ProjectEdges),
            Throws.TypeOf<InvalidDataException>());

        nodes[0] = new RepositoryGraphNode(nodes[0].ProjectId, nodes[0].ProjectPath,
            nodes[0].TargetFramework, new BuildProfile("base", new[] { "SINGLE_THREADED" }));
        Assert.That(() => RepositoryCompilationGraph.Create(nodes, fixedGraph.ProjectEdges),
            Throws.TypeOf<InvalidDataException>());
    }
    ...
    {
        Assert.That(RepositoryTargetFrameworkSelection.Select(
            new[] { "netstandard2.0", "net8.0" }, "netstandard2.0"), Is.EqualTo("netstandard2.0"));
        Assert.That(() => RepositoryTargetFrameworkSelection.Select(
            new[] { "netstandard2.0", "net8.0" }, null), Throws.TypeOf<InvalidDataException>());
        Assert.That(() => RepositoryTargetFrameworkSelection.Select(
            new[] { "netstandard2.0", "net8.0" }, "net6.0"), Throws.TypeOf<InvalidDataException>());
    }
}
