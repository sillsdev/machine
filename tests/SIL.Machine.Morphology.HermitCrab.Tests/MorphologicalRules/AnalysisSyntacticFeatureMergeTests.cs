...
internal class AnalysisSyntacticFeatureMergeTests : HermitCrabTestBase
{
    ...
    public void CategoryChangeChain_AccumulatedPosAndThirdRuleGate_MatchModeSemantics(
        AnalysisSyntacticFeatureMergeMode mode
    )
    {
    ...
    public void CategoryChangeChain_EndToEnd_FullValidChain_ReportedAcrossModes()
    {
        ...
        foreach (
            AnalysisSyntacticFeatureMergeMode mode in new[]
            {
        ...
        static bool FoundFourMorphParse(List<Word> words) =>
            words.Any(w =>
                w.AllomorphsInMorphOrder.Select(m => m.Morpheme.Gloss)
                    .SequenceEqual(new[] { "flipRoot", "a2b", "b2a", "third" })
            );
        ...
        Assert.That(
            puFound,
            Is.EqualTo(addFound),
            "Recorded finding: does PriorityUnion match Add for this valid chain? (see console output for the actual booleans)"
        );
        Assert.That(
            exactFound,
            Is.EqualTo(addFound),
            "Recorded finding: does Exact match Add for this valid chain? (see console output for the actual booleans)"
        );
    }
    ...
    public void OverrideLoss_TenseFlipFlop_AddAndPriorityUnionLoseTheParse_ExactFindsIt()
    {
        ...
        static bool FoundChain(List<Word> words) =>
            words.Any(w =>
                w.AllomorphsInMorphOrder.Select(m => m.Morpheme.Gloss)
                    .SequenceEqual(new[] { "olRoot", "olInner", "olOuter", "olOutermost" })
            );

        Console.WriteLine($"Add found chain: {FoundChain(results[AnalysisSyntacticFeatureMergeMode.Add])}");
        Console.WriteLine(
            $"PriorityUnion found chain: {FoundChain(results[AnalysisSyntacticFeatureMergeMode.PriorityUnion])}"
        );
    ...
    public void NonOverlappingNestedHeadFeatures_MergeIdenticallyInAllModes(AnalysisSyntacticFeatureMergeMode mode)
    {
        ...
        FeatureStruct tensePast = FeatureStruct
            ...
            .EqualTo(head => head.Feature("tense").EqualTo("past"))
            .Value;
        FeatureStruct numPl = FeatureStruct
            .New(Language.SyntacticFeatureSystem)
            .Feature(Head)
        ...
        AssertFSEqual(
            result.SyntacticFeatureStruct,
            expected,
            $"{mode}: non-overlapping nested features should merge the same way in every mode."
        );
    }
    ...
    public void EmptyRequiredAndOut_ClearsFS_ExceptUnderExact(AnalysisSyntacticFeatureMergeMode mode)
    {
        ...
        AssertFSEqual(
            afterX.SyntacticFeatureStruct,
            posN,
            $"{mode}: sanity check -- ruleX alone should fold in POS=N."
        );
    ...
    public void SameRuleAppliedTwice_SecondApplicationGatedDifferentlyByMode(AnalysisSyntacticFeatureMergeMode mode)
    {
        ...
        FeatureStruct numPl = FeatureStruct
            ...
            .EqualTo(head => head.Feature("num").EqualTo("pl"))
            .Value;
        FeatureStruct numSg = FeatureStruct
            .New(Language.SyntacticFeatureSystem)
            .Feature(Head)
            .EqualTo(head => head.Feature("num").EqualTo("sg"))
            .Value;
        ...
        if (mode == AnalysisSyntacticFeatureMergeMode.Add)
        {
            Assert.That(
                afterSecond,
                Is.Not.Empty,
                "Add: {sg,pl} is unifiable with Out=sg, so a second un-application is attempted."
            );
        }
        else
        {
            Assert.That(
                afterSecond,
                Is.Empty,
                $"{mode}: the narrowed accumulated value (pl) is not unifiable with Out=sg, so the second un-application is rejected."
            );
        }
    }
    ...
    public void GuessedRoot_OverrideLossBugAlsoBreaksGuessing_ButNotViaTheGuessedEntrysFS()
    {
        ...
        Console.WriteLine(
            $"PriorityUnion guess results: {results[AnalysisSyntacticFeatureMergeMode.PriorityUnion].Count}"
        );
    ...
    public void Compounding_AfterDerivationalRule_ParityAcrossModes()
    {
        ...
        static List<string> AsGlosses(List<Word> words) =>
            words
                .Select(w => string.Join(" ", w.AllomorphsInMorphOrder.Select(m => m.Morpheme.Gloss)))
                .OrderBy(s => s)
                .ToList();
        ...
        Assert.That(
            puGlosses,
            Is.EqualTo(addGlosses),
            "PriorityUnion should match Add for this simple (non-flip-flop) compounding case."
        );
        Assert.That(
            exactGlosses,
            Is.EqualTo(addGlosses),
            "Exact should match Add for this simple (non-flip-flop) compounding case."
        );
    }
    ...
    public void Compounding_FlipFlopWithOuterCheck_MirrorsTask1Finding()
    {
        ...
        Assert.That(
            FoundChain(results[AnalysisSyntacticFeatureMergeMode.Add]),
            Is.True,
            "sanity: Add should find the chain it was designed to expose."
        );
    }
    ...
    public void OverrideLoss_ThroughNonFinalTemplate_LostInAllModes_KnownLimitation()
    {
        ...
        static bool FoundChain(List<Word> words) =>
            words.Any(w =>
                w.AllomorphsInMorphOrder.Select(m => m.Morpheme.Gloss)
                    .SequenceEqual(new[] { "tlRoot", "tlInner", "tlOuter", "tlSuffix" })
            );

        Assert.That(
            FoundChain(results[AnalysisSyntacticFeatureMergeMode.Add]),
            Is.False,
            "Add: stale tense:past survives the template -- pre-existing master bug."
        );
        Assert.That(
            FoundChain(results[AnalysisSyntacticFeatureMergeMode.PriorityUnion]),
            Is.False,
            "PriorityUnion: same as Add; PR #494 does not touch the template merge."
        );
        Assert.That(
            FoundChain(results[AnalysisSyntacticFeatureMergeMode.Exact]),
            Is.False,
            "Exact: slot removes tense:past but the template merge re-Adds it (known limitation, needs FS-aware dedup)."
        );
    }
    ...
    public void ShapeMerge_LowerStratumGateUsesCanonicalFsOnly(int order, bool widen, bool narrowModesFind)
    {
        ...
        Assert.That(
            found[AnalysisSyntacticFeatureMergeMode.Add],
            Is.True,
            "master (Add) finds root+R3+R2 in every configuration"
        );
        Assert.That(
            found[AnalysisSyntacticFeatureMergeMode.PriorityUnion],
            Is.EqualTo(narrowModesFind),
            "PriorityUnion (PR #494)"
        );
        Assert.That(found[AnalysisSyntacticFeatureMergeMode.Exact], Is.EqualTo(narrowModesFind), "Exact");
    }

    private static AffixProcessRule MakeSuffixRuleIn(
