...
namespace SIL.Machine.Test.FeatureModel
{
	[TestFixture]
	public class FeatureStructTest
	{
		...
		public void Unify()
		{
			...
			TestBinaryOperation(FreezableEqualityComparer<FeatureStruct>.Default, resultsSelector, varResultsSelector,
				// simple
				featSys => null,
			...
			Assert.That(result, Is.EqualTo(FeatureStruct.New(featureSystem)
				...
				.Feature("i").ReferringTo(1).Value).Using(FreezableEqualityComparer<FeatureStruct>.Default));
		}
