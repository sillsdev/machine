...
{
	public class PoorMansAffixIdentifier<TSeq, TItem> : PoorMansStemmingAlgorithmBase<TSeq, TItem>, IAffixIdentifier<TSeq, TItem>
	{
		...
		public IEnumerable<Affix<TItem>> IdentifyAffixes(IEnumerable<TSeq> sequences, AffixType type)
		{
			...
			foreach (AffixInfo affix in affixes.Where(a => (NormalizeScores ? a.NormalizedZScore : a.ZScore) >= Threshold).OrderByDescending(a => a.ZScore))
			{
				if (results.All(a => !affix.Ngram.StartsWith(a.Ngram, type == AffixType.Prefix ? Direction.LeftToRight : Direction.RightToLeft)))
					results.Add(new Affix<TItem>(type, affix.Ngram, NormalizeScores ? affix.NormalizedZScore : affix.ZScore));
			}
