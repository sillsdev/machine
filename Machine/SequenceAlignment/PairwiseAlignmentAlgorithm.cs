...
namespace SIL.Machine.SequenceAlignment
{
	...
	public delegate IEnumerable<TItem> ItemsSelector<in TSeq, out TItem>(TSeq sequence, out int startIndex, out int count); 
	...
	public class PairwiseAlignmentAlgorithm<TSeq, TItem>
	{
		...
		public void Compute()
		{
			...
			if (Mode == AlignmentMode.Global || Mode == AlignmentMode.HalfLocal)
			{
				...
				{
					_sim[i, 0] = _sim[i - 1, 0] + _gapPenalty + _scorer.GetDeletionScore(_sequence1, Get1(i), _sequence2, default(TItem));
				}

				for (int j = 1; j < _sim.GetLength(1); j++)
				{
					_sim[0, j] = _sim[0, j - 1] + _gapPenalty + _scorer.GetInsertionScore(_sequence1, default(TItem), _sequence2, Get2(j));
				}
			}
			...
			for (int i = 1; i < _sim.GetLength(0); i++)
			{
				for (int j = 1; j < _sim.GetLength(1); j++)
				{
					int m1 = _sim[i - 1, j] + _gapPenalty + _scorer.GetDeletionScore(_sequence1, Get1(i), _sequence2, Get2(j));
					int m2 = _sim[i, j - 1] + _gapPenalty + _scorer.GetInsertionScore(_sequence1, Get1(i), _sequence2, Get2(j));
		...
		private IEnumerable<Alignment<TSeq, TItem>> GetAlignments(int threshold)
		{
			switch (Mode)
			{
				case AlignmentMode.Global:
					{
						foreach (Alignment<TSeq, TItem> alignment in GetAlignments(_sim.GetLength(0) - 1, _sim.GetLength(1) - 1, threshold))
		...
		private IEnumerable<Alignment<TSeq, TItem>> GetAlignments(int i, int j, int threshold)
		{
			...
			foreach (Tuple<List<AlignmentCell<TItem>>, List<AlignmentCell<TItem>>, int, int, int> alignment in Retrieve(i, j, 0, threshold))
			{
				...
				yield return new Alignment<TSeq, TItem>(alignment.Item5, CalcNormalizedScore(startIndex1, endIndex1, startIndex2, endIndex2, alignment.Item5),
					Tuple.Create(_sequence1, new AlignmentCell<TItem>(_items1.Take(startIndex1)), (IEnumerable<AlignmentCell<TItem>>) alignment.Item1, new AlignmentCell<TItem>(_items1.Skip(endIndex1))),
					Tuple.Create(_sequence2, new AlignmentCell<TItem>(_items2.Take(startIndex2)), (IEnumerable<AlignmentCell<TItem>>) alignment.Item2, new AlignmentCell<TItem>(_items2.Skip(endIndex2))));
			}
		}
		...
		private IEnumerable<Tuple<List<AlignmentCell<TItem>>, List<AlignmentCell<TItem>>, int, int, int>> Retrieve(int i, int j, int score, int threshold)
		{
			...
			else
			{
				...
				if (i != 0 && j != 0)
				{
					...
					{
						foreach (Tuple<List<AlignmentCell<TItem>>, List<AlignmentCell<TItem>>, int, int, int> alignment in Retrieve(i - 1, j - 1, score + opScore, threshold))
						{
				...
				if (j != 0)
				{
					...
					{
						foreach (Tuple<List<AlignmentCell<TItem>>, List<AlignmentCell<TItem>>, int, int, int> alignment in Retrieve(i, j - 1, score + opScore, threshold))
						{
				...
				if (ExpansionCompressionEnabled && i != 0 && j - 2 >= 0)
				{
					...
					{
						foreach (Tuple<List<AlignmentCell<TItem>>, List<AlignmentCell<TItem>>, int, int, int> alignment in Retrieve(i - 1, j - 2, score + opScore, threshold))
						{
							alignment.Item1.Add(new AlignmentCell<TItem>(Get1(i)));
				...
				if (i != 0)
				{
					...
					{
						foreach (Tuple<List<AlignmentCell<TItem>>, List<AlignmentCell<TItem>>, int, int, int> alignment in Retrieve(i - 1, j, score + opScore, threshold))
						{
							alignment.Item1.Add(new AlignmentCell<TItem>(Get1(i)));
				...
				if (ExpansionCompressionEnabled && i - 2 >= 0 && j != 0)
				{
					...
					{
						foreach (Tuple<List<AlignmentCell<TItem>>, List<AlignmentCell<TItem>>, int, int, int> alignment in Retrieve(i - 2, j - 1, score + opScore, threshold))
						{
		...
		private Tuple<List<AlignmentCell<TItem>>, List<AlignmentCell<TItem>>, int, int, int> CreateAlignment(int i, int j, int score)
		{
			return Tuple.Create(new List<AlignmentCell<TItem>>(), new List<AlignmentCell<TItem>>(), _startIndex1 + i, _startIndex2 + j, score);
		}
	}
}
