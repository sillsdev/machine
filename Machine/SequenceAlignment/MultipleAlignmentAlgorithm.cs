...
namespace SIL.Machine.SequenceAlignment
{
	public class MultipleAlignmentAlgorithm<TSeq, TItem>
	{
		...
		private IEnumerable<AlignmentCell<TItem>[]> GetProfileItems(Profile<TSeq, TItem> profile, out int startIndex, out int count)
		{
			startIndex = 0;
			count = profile.Alignment.ColumnCount;
			return Enumerable.Range(0, profile.Alignment.ColumnCount).Select(col => Enumerable.Range(0, profile.Alignment.SequenceCount).Select(seq => profile.Alignment[seq, col]).ToArray());
		}
