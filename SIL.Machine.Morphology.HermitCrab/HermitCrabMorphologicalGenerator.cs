...
{
	public class HermitCrabMorphologicalGenerator : IMorphologicalGenerator
	{
		private readonly ReadOnlyObservableCollection<Morphology.Morpheme> _morphemes;
		private readonly Morpher _morpher;
		private readonly Dictionary<string, Morpheme> _hcMorphemes; 

		public HermitCrabMorphologicalGenerator(Func<Morpheme, string> getMorphemeId, Func<FeatureStruct, string> getCategory, Morpher morpher)
		{
			_morpher = morpher;
			var morphemes = new ObservableList<Morphology.Morpheme>();
			...
			_morphemes = new ReadOnlyObservableCollection<Morphology.Morpheme>(morphemes);
		}

		private static Morphology.Morpheme CreateMorpheme(Func<FeatureStruct, string> getCategory, string id, LexEntry entry)
		{
			return new Morphology.Morpheme(id, getCategory(entry.SyntacticFeatureStruct), entry.Gloss, MorphemeType.Stem);
		}

		private static Morphology.Morpheme CreateMorpheme(Func<FeatureStruct, string> getCategory, string id, AffixProcessRule apr)
		{
			string category = getCategory(apr.OutSyntacticFeatureStruct) ?? getCategory(apr.RequiredSyntacticFeatureStruct);
			return new Morphology.Morpheme(id, category, apr.Gloss, MorphemeType.Affix);
		}

		private static Morphology.Morpheme CreateMorpheme(Func<FeatureStruct, string> getCategory, string id, RealizationalAffixProcessRule rapr)
		{
			string category = getCategory(rapr.RealizationalFeatureStruct) ?? getCategory(rapr.RequiredSyntacticFeatureStruct);
			return new Morphology.Morpheme(id, category, rapr.Gloss, MorphemeType.Affix);
		}

		public IReadOnlyObservableCollection<Morphology.Morpheme> Morphemes
		{
		...
		public IEnumerable<string> GenerateWords(WordAnalysis wordAnalysis)
		{
			...
			foreach (Morphology.Morpheme mi in wordAnalysis.Morphemes)
			{
				Morpheme morpheme;
