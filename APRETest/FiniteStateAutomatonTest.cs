...
namespace SIL.APRE.Test
{
	...
	public class FiniteStateAutomatonTest
	{
		...
		public void Determinize()
		{
			...
			fsa.StartState.AddArc(new Arc<int>(new ArcCondition<int>(FeatureStruct.With(featSys).Symbol("pi1+").Value), q1));
			State<int> q2 = fsa.CreateAcceptingState();
			fsa.StartState.AddArc(new Arc<int>(new ArcCondition<int>(FeatureStruct.With(featSys).Symbol("pi1+").Value), q2));
			fsa.StartState.AddArc(new Arc<int>(new ArcCondition<int>(FeatureStruct.With(featSys).Symbol("pi2+").Value), q2));
			State<int> q3 = fsa.CreateAcceptingState();
			fsa.StartState.AddArc(new Arc<int>(new ArcCondition<int>(FeatureStruct.With(featSys).Symbol("pi2+").Value), q3));
			fsa.StartState.AddArc(new Arc<int>(new ArcCondition<int>(FeatureStruct.With(featSys).Symbol("pi3+").Value), q3));
			State<int> q4 = fsa.CreateAcceptingState();
			fsa.StartState.AddArc(new Arc<int>(new ArcCondition<int>(FeatureStruct.With(featSys).Symbol("pi2+").Value), q4));
			State<int> q5 = fsa.CreateAcceptingState();
			fsa.StartState.AddArc(new Arc<int>(new ArcCondition<int>(FeatureStruct.With(featSys).Symbol("pi3+").Value), q5));
			fsa.Determinize();
