...
outcomes and rejected missing keys or signature differences. All ten arm comparisons passed the legacy
`WordAnalysisSignature` check, which on these FLEx-exported grammars compares morph counts only (`Morpheme.Id`
is empty; see `hermitcrab-perf-2026-09-disposition.md`), and the saved outputs contained zero parse or load
errors.
