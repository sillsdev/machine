...
| --- | --- | --- | --- |
| Analysis edge-segment prefilter | ~120 in 2 rule files + 9 in `Morpher` + tests | 87% fewer FST traversals on 45 Sena words (deterministic counts); one invariant; internal toggle | **Isolated onto master** as `perf/hc-edge-prefilter` (`be7f241d`), see below — **that standalone branch and its own archive tag were later deleted, but the mechanism is preserved in the whole-round archive branch `perf/hc-optimization-archive`** |
...
**Status update:** `perf/hc-edge-prefilter` was deleted after this round closed, and its own archive tag was
later deleted too, but the mechanism is not lost: it remains on the whole-round archive
branch `perf/hc-optimization-archive`, which still carries
`Morpher.EdgePrefilterEnabled`, the edge constraints in `AnalysisAffixProcessRule`, and
`AnalysisEdgePrefilterTests`. The description below is kept as a standalone record of the isolated, reviewable
form, in case that smaller diff is preferred over pulling the mechanism out of the full archive again.
...
branches), and do not spend review budget on it ahead of #491. The standalone single-commit branch is deleted
(see status note above); a PR now starts from the mechanism preserved in the whole-round archive
(branch `perf/hc-optimization-archive`) rather than resuming
that branch.
...
| `perf/hc-edge-prefilter` | `be7f241d` | the one isolated mechanism on `origin/master` | optional small PR at the time; **branch and its own archive tag have both since been deleted, but the mechanism is preserved on branch `perf/hc-optimization-archive`** |
...
| `docs/hc-optimization-ledger` (remote, PR #490) | `cc806613` | the ledger, now 23 rows | open docs PR |
...
Records in this directory: `hermitcrab-optimization-ledger.md` (23 attempts), `hermitcrab-perf-2026-09-counts.md`
(count round), `hermitcrab-probe-design.md`, `optimizations/*` and `rejected-optimizations/*` (one record per
mechanism), and this file. The round's execution checklist has been folded into this record; it is not kept as
...
3. ~~Open the prefilter PR from `perf/hc-edge-prefilter`~~ — that branch and its own archive tag are both
   deleted; a PR now starts from the mechanism preserved on branch
   `perf/hc-optimization-archive` rather than resuming the deleted branch.
