...
public class S3WriteStream : Stream
{
    ...
    public override async Task WriteAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if(response.HttpStatusCode != HttpStatusCode.OK) throw new HttpRequestException($"Uploading part {partNumber} of upload {_uploadId} to {_bucketName}/{_key} but received response code {response.HttpStatusCode}")
