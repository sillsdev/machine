...
public class SmtTransferEngineService : TranslationEngineServiceBase<SmtTransferEngineBuildJob>
{
    ...
    {
        IEnumerable<TranslationEngine> engines = await Engines.GetAllAsync(
            e => e.Id == engineId && e.BuildRevision > 0 && e.BuildState == BuildState.None
        );
        foreach (var eng in engines)
            Console.WriteLine(JsonSerializer.Serialize(eng));
        if (!engines.Any())
