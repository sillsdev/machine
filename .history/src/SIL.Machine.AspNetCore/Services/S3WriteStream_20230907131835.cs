...
public class S3WriteStream : Stream
{
    ...
    public override async Task WriteAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
    {
        try
        {
            ...
            request.StreamTransferProgress += new EventHandler<StreamTransferProgressArgs>((_, e) => { });
            UploadPartResponse response = await _client.UploadPartAsync(request);
            if (response.HttpStatusCode != HttpStatusCode.OK)
                throw new HttpRequestException(
                    $"Uploading part {partNumber} of upload {_uploadId} to {_bucketName}/{_key} but received response code {response.HttpStatusCode}"
                );
