...
namespace SIL.Machine.Morphology.HermitCrab
{
    ...
    /// ops minus the ops the sibling generators cover. It is a cheap, build-time coverage diagnostic —
    /// which construct types this composite claims to handle at all — not a soundness or completeness
    /// guarantee; a generator that covers an op only partially still under-generates on some words for
    /// that op (verify + the probe's own gap diagnostics are what actually reveal that).
    /// </summary>
    public class CompositeProposer : IMorphologicalAnalyzer
    {
        ...
        /// generators, the phonology-composition proposer (Point 4, all bounded phonology including
        /// cross-boundary), and the lockstep-composition phonology proposer (LEVER_2.md/
        /// FST_FAST_PATH_PLAN.md Phase 3 — the lexicon-constrained auto-compiled Pinv walk; narrower
        /// coverage than the composition proposer today, additive not a replacement, see
        /// <see cref="LockstepPhonologyProposer"/>). For a grammar without a given construct the
        /// corresponding generator is inert (it holds no rules and yields nothing — both phonology
        /// proposers short-circuit when the grammar has no matching phonological rules), so this adds
        /// near-zero overhead and does not change behavior; that is why the factories wire it
        ...
        )
        {
            var generators = new List<IConstructProposer>
            {
                ...
                new LockstepPhonologyProposer(language, new Morpher(new TraceManager(), language)),
            };
            if (forwardSynthesis)
            {
                generators.Insert(
        ...
        /// A coverage diagnostic only — see class remarks.</summary>
