...
namespace SIL.Machine.Morphology.HermitCrab
{
    ...
    /// surface-variants)` candidates — bounded.
    ///
    /// Surface variants: an infix whose own surface is phonologically altered (e.g. it devoices between
    /// two consonants) would otherwise never be found by a literal search for its underlying form, so
    /// each infix's <see cref="SurfacePhonology"/> variants (the same isolation/boundary-probe precompile
    /// built for regular affix arcs) are searched too — the underlying form is always included, so a
    /// 0-phonology grammar is unaffected.
    ///
    /// Scope (first cut): the infix must be a single contiguous run of inserted segments. Templatic
    /// multi-slot infixes (separate insert runs) are left to the engine (the parity gate keeps results
    ...
    public class InfixProposer : IConstructProposer
    {
        ...
        private readonly List<(MorphemicMorphologicalRule Rule, IReadOnlyCollection<string> Surfaces)> _infixes;

        public InfixProposer(Language language, IMorphologicalAnalyzer baseProposer)
        {
            _baseProposer = baseProposer;
            _infixes = new List<(MorphemicMorphologicalRule, IReadOnlyCollection<string>)>();
            var surfacePhonology = new SurfacePhonology(language, new Morpher(new TraceManager(), language));
            foreach (Stratum stratum in language.Strata)
            {
                foreach (IMorphologicalRule mrule in stratum.MorphologicalRules)
                {
                    ...
                    foreach (AffixProcessAllomorph allomorph in rule.Allomorphs)
                    {
                        ...
                        {
                            _infixes.Add((rule, surfacePhonology.Variants(infix)));
                        }
                    }
                }
            }
        ...
        {
            foreach ((MorphemicMorphologicalRule rule, IReadOnlyCollection<string> surfaces) in _infixes)
            {
                foreach (string infix in surfaces)
                {
                    ...
                    while (i >= 1 && i + infix.Length < word.Length)
                    {
                        ...
                        {
                            var morphemes = new List<IMorpheme>(baseAnalysis.Morphemes) { rule };
