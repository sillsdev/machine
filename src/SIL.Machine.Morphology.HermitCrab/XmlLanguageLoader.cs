...
namespace SIL.Machine.Morphology.HermitCrab
{
    ...
    public class XmlLanguageLoader
    {
            ...
            public override object GetEntity(Uri absoluteUri, string role, Type ofObjectToReturn)
            {
                ...
                if (fileName == "HermitCrabInput.dtd")
                    return GetType().Assembly.GetManifestResourceStream(
                        "SIL.Machine.Morphology.HermitCrab.HermitCrabInput.dtd"
                    );
        ...
        private void LoadLanguage(XElement langElem)
        {
            ...
            IEnumerable<FeatureSymbol> posSymbols = langElem
                .Elements("PartsOfSpeech")
            ...
            foreach (
                XElement mfElem in langElem
                    .Elements("MorphologicalPhonologicalRuleFeatures")
                    .Elements("MorphologicalPhonologicalRuleFeature")
                    .Where(IsActive)
            )
            {
            ...
            foreach (
                XElement mprFeatGroupElem in langElem
                    .Elements("MorphologicalPhonologicalRuleFeatures")
                    .Elements("MorphologicalPhonologicalRuleFeatureGroup")
                    .Where(IsActive)
            )
            {
            ...
            foreach (
                XElement coOccurElem in langElem
                    .Elements("MorphemeCoOccurrenceRules")
                    .Elements("MorphemeCoOccurrenceRule")
                    .Where(IsActive)
            )
            {
            ...
            foreach (
                XElement coOccurElem in langElem
                    .Elements("AllomorphCoOccurrenceRules")
                    .Elements("AllomorphCoOccurrenceRule")
                    .Where(IsActive)
            )
            {
        ...
        private void LoadStratum(XElement stratumElem)
        {
            ...
            foreach (
                XElement mruleElem in stratumElem.Elements("MorphologicalRuleDefinitions").Elements().Where(IsActive)
            )
            {
            ...
            foreach (
                XElement tempElem in stratumElem.Elements("AffixTemplates").Elements("AffixTemplate").Where(IsActive)
            )
            {
            ...
            foreach (
                XElement entryElem in stratumElem.Elements("LexicalEntries").Elements("LexicalEntry").Where(IsActive)
            )
            {
                LexEntry entry;
        ...
        private RootAllomorph LoadRootAllomorph(XElement alloElem, CharacterDefinitionTable table)
        {
            ...
            allomorph.Environments.AddRange(
                LoadAllomorphEnvironments(alloElem.Element("RequiredEnvironments"), ConstraintType.Require, table)
            );
            allomorph.Environments.AddRange(
        ...
        private IEnumerable<AllomorphEnvironment> LoadAllomorphEnvironments(
            XElement envsElem,
            ConstraintType type,
        ...
        private Feature LoadFeature(XElement featElem)
        {
            ...
            switch (featElem.Name.LocalName)
            {
                case "SymbolicFeature":
                    IEnumerable<FeatureSymbol> symbols = featElem
                        .Elements("Symbols")
                        .Elements("Symbol")
        ...
        private void LoadCharacterDefinitionTable(XElement charDefTableElem)
        {
            ...
            foreach (
                XElement segDefElem in charDefTableElem
                    .Elements("SegmentDefinitions")
                    .Elements("SegmentDefinition")
                    .Where(IsActive)
            )
            {
                IEnumerable<string> reps = segDefElem
        ...
        private void LoadRewriteRule(XElement pruleElem)
        {
            ...
            prule.Lhs = LoadPhoneticSequence(
                pruleElem.Elements("PhoneticInput").Elements("PhoneticSequence").SingleOrDefault(),
                variables
            );

            foreach (
                XElement subruleElem in pruleElem
                    .Elements("PhonologicalSubrules")
                    .Elements("PhonologicalSubrule")
                    .Where(IsActive)
            )
            {
        ...
        private RewriteSubrule LoadRewriteSubrule(
            XElement psubruleElem,
        ...
        )
        {
            ...
            subrule.ExcludedMprFeatures.UnionWith(
                LoadMprFeatures((string)psubruleElem.Attribute("excludedMPRFeatures"))
            );

            subrule.Rhs = LoadPhoneticSequence(
                psubruleElem.Elements("PhoneticOutput").Elements("PhoneticSequence").SingleOrDefault(),
                variables
            );
        ...
        private void LoadMetathesisRule(XElement metathesisElem)
        {
            ...
            metathesisRule.Pattern = LoadPhoneticTemplate(
                ...
                new Dictionary<string, Tuple<string, SymbolicFeature>>(),
                null,
                groupNames
            );
        ...
        private bool TryLoadAffixProcessRule(
            XElement mruleElem,
            CharacterDefinitionTable defaultTable,
        ...
        )
        {
            ...
            foreach (
                XElement subruleElem in mruleElem
                    .Elements("MorphologicalSubrules")
                    .Elements("MorphologicalSubrule")
                    .Where(IsActive)
            )
            {
                try
        ...
        private bool TryLoadRealizationalRule(
            XElement realRuleElem,
            CharacterDefinitionTable defaultTable,
            out IMorphologicalRule mrule
        )
        {
            ...
            foreach (
                XElement subruleElem in realRuleElem
                    .Elements("MorphologicalSubrules")
                    .Elements("MorphologicalSubrule")
                    .Where(IsActive)
            )
        ...
        private AffixProcessAllomorph LoadAffixProcessAllomorph(
            XElement msubruleElem,
            CharacterDefinitionTable defaultTable
        )
        {
            ...
            allomorph.Environments.AddRange(
                LoadAllomorphEnvironments(
                    msubruleElem.Element("RequiredEnvironments"),
            ...
            allomorph.Environments.AddRange(
                LoadAllomorphEnvironments(
                    msubruleElem.Element("ExcludedEnvironments"),
            ...
            allomorph.RequiredMprFeatures.UnionWith(
                LoadMprFeatures((string)inputElem.Attribute("requiredMPRFeatures"))
            );
            allomorph.ExcludedMprFeatures.UnionWith(
                LoadMprFeatures((string)inputElem.Attribute("excludedMPRFeatures"))
            );
        ...
        private void LoadMorphologicalLhs(
        ...
        private void LoadMorphologicalRhs(
            XElement phonOutputElem,
            Dictionary<string, Tuple<string, SymbolicFeature>> variables,
            Dictionary<string, string> partNames,
            ...
            foreach (XElement partElem in phonOutputElem.Elements())
            {
                switch (partElem.Name.LocalName)
                {
                    ...
                    case "InsertSimpleContext":
                        rhs.Add(
                            new InsertSimpleContext(LoadSimpleContext(partElem.Element("SimpleContext"), variables))
                        );
                        break;
                    ...
                    case "ModifyFromInput":
                        rhs.Add(
                            new ModifyFromInput(
                                partNames[(string)partElem.Attribute("index")],
        ...
        private bool TryLoadCompoundingRule(
            XElement compRuleElem,
            CharacterDefinitionTable defaultTable,
            out IMorphologicalRule mrule
        )
        {
            ...
            foreach (
                XElement subruleElem in compRuleElem
                    .Elements("CompoundingSubrules")
                    .Elements("CompoundingSubrule")
                    .Where(IsActive)
            )
            {
                try
        ...
        private CompoundingSubrule LoadCompoundingSubrule(
            XElement compSubruleElem,
            CharacterDefinitionTable defaultTable
        )
        {
