...
namespace SIL.Machine.Morphology.HermitCrab
{
    ...
    ///
    /// Because the inverse is applied to the <i>assembled</i> surface, this covers a large share of
    /// bounded phonology — including the cross-boundary, stem-conditioned alternations the
    /// per-morpheme precompile (Point 1) cannot see. The un-applied shape carries under-specified
    /// nodes (analysis is non-deterministic), which the unification walk matches against every
    /// compatible arc; verify prunes the spurious ones, so it stays a sound superset. An unbounded
    /// self-feeding cycle is not a regular relation and simply will not be found this way — those
    /// words stay unparsed by design, not incorrectly parsed.
    ///
    /// <b>Thread-safe.</b> The inverse cascade is compiled once against a private <see cref="Morpher"/>
    /// with its own <see cref="TraceManager"/> (not the caller's), and each <see cref="AnalyzeWord"/>
    /// applies it to a fresh local <see cref="Word"/> — no per-call mutation of shared state — so the
    /// composite stays safe on the parallel path <see cref="VerifiedFstAnalyzer"/> supports.
    /// </summary>
    public class ComposedPhonologyProposer : IConstructProposer
    {
        ...
        private readonly LinearRuleCascade<Word, int> _inverse;
        ...
        public ComposedPhonologyProposer(Language language, FstTemplateAnalyzer fst)
        {
            ...
            var rules = new List<IRule<Word, int>>();
            ...
            _inverse = new LinearRuleCascade<Word, int>(rules);
        }
