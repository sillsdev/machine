...
namespace SIL.Machine.Morphology.HermitCrab
{
    ...
    public static class GrammarFstAdvisor
    {
        ...
        {
            Pattern<Word, int> part = allomorph.Lhs.FirstOrDefault(p => p.Name == partName);
            if (part == null)
                return false;
            return !HasUnboundedQuantifier(part);
        }

        private static bool HasUnboundedQuantifier(Pattern<Word, int> pattern)
        {
            if (pattern == null)
                return false;
            return pattern
                .GetNodesDepthFirst()
                .OfType<Quantifier<Word, int>>()
                .Any(q => q.MaxOccur == Quantifier<Word, int>.Infinite);
        }

        private static int CountConstraints(Pattern<Word, int> pattern)
        {
            if (pattern == null)
                return 0;
            return pattern.GetNodesDepthFirst().OfType<Constraint<Word, int>>().Count();
        }
    }
}
