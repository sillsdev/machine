...
public class BuildService : EntityServiceBase<Build>, IBuildService
{
	...
	public Task<Build?> GetActiveAsync(string parentId, CancellationToken cancellationToken = default)
	{
		...
		return Entities.GetAsync(b => b.ParentRef == parentId
			&& (b.State == BuildState.Active || b.State == BuildState.Pending), cancellationToken);
	}

	public Task<EntityChange<Build>> GetNewerRevisionAsync(string id, long minRevision,
	...
	public Task<EntityChange<Build>> GetActiveNewerRevisionAsync(string parentId, long minRevision,
		CancellationToken cancellationToken = default)
	{
		CheckDisposed();

		return GetNewerRevisionAsync(b => b.ParentRef == parentId
			&& (b.State == BuildState.Active || b.State == BuildState.Pending), minRevision, cancellationToken);
	}

	private async Task<EntityChange<Build>> GetNewerRevisionAsync(Expression<Func<Build, bool>> filter,
		long minRevision, CancellationToken cancellationToken = default)
	{
