...
public class CorpusService : EntityServiceBase<Corpus>, ICorpusService
{
	...
	public CorpusService(IRepository<Corpus> corpora, IOptions<CorpusOptions> corpusOptions)
		: base(corpora)
	{
	...
	public async Task<ITextCorpus?> CreateTextCorpusAsync(string id, string languageTag)
	{
		...
		switch (corpus.Format)
		{
			case FileFormat.Text:
				textCorpus = new DictionaryTextCorpus(files
					.Select(f => new TextFileText(f.TextId ?? f.Name, GetDataFilePath(f))));
				break;
