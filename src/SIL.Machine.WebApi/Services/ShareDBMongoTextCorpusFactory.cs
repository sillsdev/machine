...
namespace SIL.Machine.WebApi.Services
{
	public class ShareDBMongoTextCorpusFactory : ITextCorpusFactory
	{
		...
		private IEnumerable<ShareDBMongoText> CreateTexts(IEnumerable<string> projects, ITokenizer<string, int> wordTokenizer,
			TextCorpusType type)
		{
			...
			{
				IMongoCollection<BsonDocument> collection = db.GetCollection<BsonDocument>(projectId);
				...
				foreach (BsonDocument doc in cursor.ToEnumerable())
					yield return new ShareDBMongoText(projectId, doc, wordTokenizer);
			}
		}
	}
}
