...
public class DataFileService : IDataFileService
{
    ...
    public async Task<IReadOnlyDictionary<string, ITextCorpus>> CreateTextCorporaAsync(
        string engineId,
        ...
        foreach (IGrouping<string?, DataFile> corpusGrouping in dataFiles.GroupBy(f => f.CorpusKey))
        {
            ...
            switch (format)
            {
                case FileFormat.Text:
                    corpus = new DictionaryTextCorpus(
                        corpusDataFiles.Select(f => new TextFileText(f.Name, GetDataFilePath(f)))
                    );
                    break;
