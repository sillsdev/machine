...
public class MongoSubscription<T> : DisposableBase, ISubscription<T> where T : IEntity
{
	...
	public MongoSubscription(IMongoCollection<T> entities, IMongoCollection<ChangeEvent> changeEvents,
		Func<T, bool> filter, T? initialEntity)
	{
		_entities = entities;
	...
	public async Task WaitForChangeAsync(TimeSpan? timeout = default, CancellationToken cancellationToken = default)
	{
		...
		using IAsyncCursor<ChangeEvent> cursor = await _changeEvents.Find(changeEventFilter,
			new FindOptions { MaxAwaitTime = timeout, CursorType = CursorType.TailableAwait })
		...
		while (await cursor.MoveNextAsync(cancellationToken))
		{
			...
			{
				T? entity = await _entities.AsQueryable().FirstOrDefaultAsync(e => e.Id == ce.EntityRef
					&& e.Revision == ce.Revision, cancellationToken);
				if (entityNotFound)
				{
