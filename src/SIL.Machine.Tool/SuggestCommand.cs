...
public class SuggestCommand : CommandBase
{
    ...
    protected override async Task<int> ExecuteCommandAsync(CancellationToken cancellationToken)
    {
        ...
        double fullPercent = (double)_fullSuggestionCount / _totalAcceptedSuggestionCount;
        Out.WriteLine($"-Full: {fullPercent:0.0000}");
        double initPercent = (double)_initSuggestionCount / _totalAcceptedSuggestionCount;
        Out.WriteLine($"-Initial: {initPercent:0.0000}");
        double finalPercent = (double)_finalSuggestionCount / _totalAcceptedSuggestionCount;
        Out.WriteLine($"-Final: {finalPercent:0.0000}");
        double middlePercent = (double)_middleSuggestionCount / _totalAcceptedSuggestionCount;
        Out.WriteLine($"-Middle: {middlePercent:0.0000}");
        Out.WriteLine("Correct Suggestion N");
        for (int i = 0; i < _acceptedSuggestionCounts.Length; i++)
        {
            double percent = (double)_acceptedSuggestionCounts[i] / _totalAcceptedSuggestionCount;
            Out.WriteLine($"-{i + 1}: {percent:0.0000}");
        }
    ...
    private static void WritePrefix(StreamWriter traceWriter, string suggestionResult, string prefix)
    {
    ...
    private static void WriteSuggestions(StreamWriter traceWriter, IReadOnlyList<TranslationSuggestion> suggestions)
    {
        if (traceWriter == null)
            return;
    ...
    private static bool SuggestionsAreEqual(string[][] x, string[][] y)
    {
