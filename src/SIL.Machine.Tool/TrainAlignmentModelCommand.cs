...
public class TrainAlignmentModelCommand : CommandBase
{
    ...
    protected override async Task<int> ExecuteCommandAsync(CancellationToken cancellationToken)
    {
        ...
        var parameters = _trainParamsOption
        ...
        using (ConsoleProgressBar progress = _quietOption.HasValue() ? null : new ConsoleProgressBar(Out))
        {
            ...
            if (_modelSpec.IsSymmetric)
                phases = [new Phase("Training model", 0.96), new Phase("Saving model", 0.04)];
            else
                phases =
                [
            ...
            using (
                ITrainer trainer = _modelSpec.CreateAlignmentModelTrainer(
                    corpus,
            ...
            if (!_modelSpec.IsSymmetric)
            {
                using ITrainer trainer = _modelSpec.CreateAlignmentModelTrainer(
                    corpus,
                    _corpusSpec.MaxCorpusCount,
                    parameters,
                    direct: false
