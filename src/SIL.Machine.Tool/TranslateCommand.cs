...
public class TranslateCommand : CommandBase
{
    ...
    public TranslateCommand()
    {
        ...
        _refFormatOption = Option(
            "-rf|--reference-format <CORPUS_FORMAT>",
            "The reference corpus format.\nFormats: \"text\" (default), \"dbl\", \"usx\", \"pt\".",
            CommandOptionType.SingleValue
        );
        _refWordTokenizerOption = Option(
            "-rt|--ref-tokenizer <TOKENIZER>",
    ...
    protected override async Task<int> ExecuteCommandAsync(CancellationToken cancellationToken)
    {
        ...
        {
            translations = [];
            ITextCorpus refCorpus = ToolHelpers.CreateTextCorpus(
                _refFormatOption.Value() ?? "text",
                _refOption.Value()
            );
        ...
        using (ITranslationModel model = _modelSpec.CreateModel())
        {
            ...
            using (StreamWriter writer = ToolHelpers.CreateStreamWriter(_outputArgument.Value))
            {
                ...
                foreach (TextRow row in corpus)
                {
                    if (IsValid(row))
                    {
                        TranslationResult translateResult = await model.TranslateAsync(row.Segment, cancellationToken);
