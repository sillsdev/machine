...
namespace SIL.Machine.Translation.Thot
{
	public class ThotWordAlignmentModelTrainer : DisposableBase, ITrainer
	{
		...
		private readonly IParallelTextCorpusView _parallelCorpus;
		...
		public ThotWordAlignmentModelTrainer(ThotWordAlignmentModelType modelType, IParallelTextCorpusView corpus,
			...
			else
			{
				...
				if (modelType >= ThotWordAlignmentModelType.Ibm2)
				{
					if (parameters.GetHmmIterationCount(modelType) > 0)
					{
						...
						{
							Thot.swAlignModel_setHmmLexicalSmoothingFactor(ibm2OrHmm,
								parameters.HmmLexicalSmoothingFactor.Value);
						}
						if (parameters.HmmAlignmentSmoothingFactor != null)
						{
							Thot.swAlignModel_setHmmAlignmentSmoothingFactor(ibm2OrHmm,
				...
				if (modelType >= ThotWordAlignmentModelType.Ibm3 && ibm2OrHmm != IntPtr.Zero
					&& parameters.GetIbm3IterationCount(modelType) > 0)
				{
					...
					{
						Thot.swAlignModel_setIbm3FertilitySmoothingFactor(ibm3,
				...
				if (modelType >= ThotWordAlignmentModelType.Ibm4)
				{
					...
					if (ibm4 != IntPtr.Zero)
					{
						...
						{
							Thot.swAlignModel_setIbm4DistortionSmoothingFactor(ibm4,
		...
		public void Train(IProgress<ProgressStatus> progress = null, Action checkCanceled = null)
		{
			...
			else
			{
				int corpusCount = 0;
				int index = 0;
				foreach (ParallelTextCorpusRow segment in _parallelCorpus.GetRows())
				{
		...
		private bool IsSegmentValid(ParallelTextCorpusRow row)
		{
			return !row.IsEmpty && row.SourceSegment.Count <= _maxSegmentLength
				&& row.TargetSegment.Count <= _maxSegmentLength;
		}

		private void AddSegmentPair(ParallelTextCorpusRow row)
		{
		...
		private static void AddWordClasses(IntPtr swAlignModelHandle, IReadOnlyDictionary<string, string> wordClasses,
			bool isSource)
		{
