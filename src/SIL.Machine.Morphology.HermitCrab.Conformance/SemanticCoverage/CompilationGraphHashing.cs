...
internal static class CompilationGraphHashing
{
    ...
    private static object ToGraphInputDocument(CompilationGraphHashInputs inputs) =>
        new
        {
            schemaVersion = inputs.SchemaVersion,
            projects = inputs
                .Projects.OrderBy(project => project.Id, StringComparer.Ordinal)
                .Select(project => new
                {
                    id = project.Id,
            ...
            profiles = inputs
                .Profiles.OrderBy(profile => profile.Id, StringComparer.Ordinal)
                .Select(profile => new
                {
                    id = profile.Id,
            ...
            edges = inputs
                .Edges.OrderBy(edge => edge.FromProjectId, StringComparer.Ordinal)
    ...
    private static object ToToolchainDocument(ToolchainHashInput toolchain) =>
        new
        {
    ...
    private static object ToGraphDocument(CompilationGraphHashInputs inputs) =>
        new
        {
            schemaVersion = inputs.SchemaVersion,
            profiles = inputs
            ...
            nodes = inputs
                .Nodes.OrderBy(node => NodeIdentity(node.Key), StringComparer.Ordinal)
                .Select(node => new
                {
    ...
    private static object ToNodeDocument(NodeHashInput node) =>
        new
        {
            ...
            arguments = node
                .Arguments.OrderBy(argument => argument.Ordinal)
                .Select(argument => new { ordinal = argument.Ordinal, value = argument.Value }),
            sources = node
                .Sources.OrderBy(source => source.Ordinal)
                .Select(source => new { ordinal = source.Ordinal, file = ToFileDocument(source.File) }),
            references = node
                .References.OrderBy(reference => reference.Identity, StringComparer.Ordinal)
            ...
            projectReferences = node
                .ProjectReferences.OrderBy(reference => reference.ProjectId, StringComparer.Ordinal)
                .Select(reference => new { projectId = reference.ProjectId, metadata = reference.Metadata }),
            analyzers = node
                .Analyzers.OrderBy(analyzer => analyzer.Identity, StringComparer.Ordinal)
                .Select(analyzer => new { identity = analyzer.Identity, file = ToFileDocument(analyzer.File) }),
            additionalFiles = node
                .AdditionalFiles.OrderBy(file => file.LogicalPath, StringComparer.Ordinal)
                .Select(ToFileDocument),
            editorConfigFiles = node
                .EditorConfigFiles.OrderBy(file => file.LogicalPath, StringComparer.Ordinal)
                .Select(ToFileDocument),
    ...
    private static object ToFileDocument(GraphHashFile file) =>
        new
        {
