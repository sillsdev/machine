...
public static class EngineGateInventoryLedger
{
    ...
    )
    {
        ...
        ["ObligatorySyntacticFeatures"] =
            "MorphologicalRule.outputObligatoryFeatures;CompoundingRule.outputObligatoryFeatures",
        ...
        ["RequiredSyntacticFeatureStruct"] =
            "MorphologicalRule.requiredPartsOfSpeech;PhonologicalSubrule.requiredPartsOfSpeech",
        ...
        ["ExcludedMprFeatures"] =
            "MorphologicalInput.excludedMPRFeatures;HeadMorphologicalInput.excludedMPRFeatures;PhonologicalSubrule.excludedMPRFeatures",
    ...
    public static string ToText(IReadOnlyList<Row> rows)
    {
        ...
        writer.WriteLine("# HermitCrab.FailureReason member (excluding None) -- HermitCrab's OWN enumeration of the");
        writer.WriteLine("# decisions it makes when declining to apply or unapply something, kept alongside (never");
        writer.WriteLine(
        ...
        writer.WriteLine("# fuses into one attribute and names several gates no DTD attribute pair expresses at all.");
        writer.WriteLine("# raise_sites is mechanically scanned (RaiseSiteScanner); dtd_attributes is hand-verified");
        writer.WriteLine(
            "# against XmlLanguageLoader.cs (see EngineGateInventoryLedger.DtdAttributes' own doc comment) and"
        );
        writer.WriteLine("# '-' is a genuine finding for six gates, not a placeholder. triggered_by_fixtures/");
        writer.WriteLine(
        ...
        writer.WriteLine("# cell's same-word pair witness (see this file's own doc comment, and conformance/docs/");
