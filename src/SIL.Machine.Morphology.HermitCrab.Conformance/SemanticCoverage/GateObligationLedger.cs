...
public static class GateObligationLedger
{
    ...
    private sealed record WordBaseline(
        bool Succeeded,
        IReadOnlySet<string> ObservedGates,
        IReadOnlySet<string> FiredRuleIds
    );
    ...
    public static IReadOnlyList<Row> Compute(string repositoryRoot)
    {
        ...
        var wordBaselineCache = new Dictionary<string, IReadOnlyDictionary<string, WordBaseline>>(
        ...
        foreach (FailureReason reason in Enum.GetValues<FailureReason>())
        {
            ...
            EngineGateInventoryLedger.Row gateRow =
                ...
                ?? throw new InvalidOperationException(
                    $"{EngineGateInventoryLedger.RelativePath} has no row for {gate}"
                );
    ...
    private static ArmResult NotEvidencedArm(string reason) =>
        new(GateArmStatus.NotEvidenced, NoValue, NoValue, reason);
    ...
    public static string ToText(IReadOnlyList<Row> rows)
    {
        ...
        writer.WriteLine("# GENERATED by hc-conformance --write-gate-obligations. One row per (gate, arm) pair, gate");
        writer.WriteLine(
            "# ranging over all 23 SIL.Machine.Morphology.HermitCrab.FailureReason members (excluding None)"
        );
        writer.WriteLine("# and arm in {Blocked, Control} -- the two arms MC/DC demands for each of the engine's own");
        writer.WriteLine("# decision points, as opposed to conformance/dataflow-obligations.tsv's DTD-attribute-pair");
        writer.WriteLine(
            "# denominator (kept, not superseded -- see conformance/docs/how-it-is-computed.md). xml_reachable"
        );
        writer.WriteLine("# and flex_producible are the two layer verdicts a gate must clear to be worth_covering;");
        writer.WriteLine(
        ...
        writer.WriteLine("gate\tarm\txml_reachable\tflex_producible\tworth_covering\tstatus\tfixture\tword\tevidence");
