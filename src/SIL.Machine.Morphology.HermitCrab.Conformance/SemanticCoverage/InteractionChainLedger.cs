...
/// </summary>
public sealed record ChainJunction(string PayloadType, IReadOnlyList<(string Element, string Attribute)> Writers, IReadOnlyList<(string Element, string Attribute)> Readers);
...
public static class InteractionChainLedger
{
    ...
    public static IReadOnlyList<Row> Compute(string repositoryRoot)
    {
        ...
        return rows
            .OrderBy(r => r.PayloadType, StringComparer.Ordinal)
    ...
    public static string ToText(IReadOnlyList<Row> rows)
    {
        ...
        writer.WriteLine(
            "# GENERATED by hc-conformance --write-interaction-chains. One row per (writer edge, payload"
        );
        writer.WriteLine(
            "# type, reader edge) at each ChainJunction -- a payload type reached by both a write- and a"
        );
        writer.WriteLine(
        ...
        writer.WriteLine(
            "# so an entirely-untested reader still appears in its own denominator instead of silently"
        );
        writer.WriteLine(
            "# vanishing. exercising_fixtures lists every fixture where the SAME id appears in both the"
        );
        writer.WriteLine(
            "# writer's and the reader's attribute values; hazardous means at least one of those fixtures"
        );
        writer.WriteLine(
            "# also has a MutatingConstructs entry (today: an overwrite-type"
        );
    ...
    public static IReadOnlyList<Row> Read(string repositoryRoot)
    {
        ...
        foreach (string raw in File.ReadAllLines(path))
        {
            ...
            rows.Add(
                new Row(fields[0], fields[1], fields[2], fields[3], fields[4], fields[5] == "yes", fixtures, fields[7] == "yes")
            );
        }
