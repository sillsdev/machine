...
/// <summary>
/// The checked-in denominator for the data-flow/MC/DC obligation-matrix layer, built on top of
/// <see cref="InteractionChainLedger"/>'s chains.
///
/// <para>
...
/// unblocks it -- the textbook gated-form shape) and the reader
...
public static class DataflowObligationLedger
{
    ...
    /// verification narrows AND widens that guess, and both corrections are load-bearing (a mutator
    /// that structurally cannot change a parse for a given chain must not generate an obligation for
    /// it):
    /// <list type="bullet">
    ...
    /// derivation-accumulated state this generator's chains model, and outside this generator's
    /// analysis-direction scope besides.</item>
    ...
    public static IReadOnlyList<Row> Compute(string repositoryRoot)
    {
        ...
        foreach (InteractionChainLedger.Row chain in chains)
        {
            ...
            if (conditionScan is { MaxTokenCount: > 1 } scan)
            {
                ...
                // control/gated-form axis that attributes the outcome.
    ...
    public static string ToText(IReadOnlyList<Row> rows)
    {
        ...
        writer.WriteLine(
            "# this file's own obligation matrix -- all-uses, plus MC/DC on every gate, plus every kill-path"
        );
        writer.WriteLine(
            "# witnessed -- built on conformance/interaction-chains.tsv (InteractionChainLedger). Every chain"
        );
        writer.WriteLine(
