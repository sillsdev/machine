...
public static class ConstructClaimCorroboration
{
    ...
    )
    {
        ...
        foreach (string construct in constructs)
        {
            ...
            result[construct] = matched.Distinct(StringComparer.Ordinal).OrderBy(t => t, StringComparer.Ordinal).ToArray();
        }
        return result;
    }
        ...
        foreach (Fixture fixture in Fixture.DiscoverAll(Path.Combine(repositoryRoot, "conformance")))
        {
            ...
            ConstructClaimStatus StatusFor(string construct)
            {
                ...
                IReadOnlyList<string> tokens = mapping.TryGetValue(construct, out IReadOnlyList<string>? found)
                    ? found
                    : Array.Empty<string>();
                ConstructClaimStatus status = tokens.Count == 0
                    ? ConstructClaimStatus.Unmapped
                    : GrammarContainsAnyToken(grammar, tokens)
                        ? ConstructClaimStatus.Confirmed
    ...
    public static string ToText(IReadOnlyList<Row> rows)
    {
        ...
        writer.WriteLine(
            "# GENERATED by hc-conformance --write-coverage-traceability. One row per words.yaml"
        );
        writer.WriteLine(
            "# 'exercises:' claim (self-reported by the fixture author), cross-checked against that"
        );
        writer.WriteLine(
        ...
        )
        {
            writer.WriteLine(
                string.Join(
                    ...
                    row.MatchedTokens.Count == 0 ? NullField : string.Join(",", row.MatchedTokens)
                )
            );
        }
    ...
    public static IReadOnlyList<Row> Read(string repositoryRoot)
    {
        ...
        foreach (string raw in File.ReadAllLines(path))
        {
            ...
            IReadOnlyList<string> tokens = fields[5] is "" or NullField
                ? Array.Empty<string>()
                : fields[5].Split(',');
            rows.Add(
                new Row(
