...
.Count} word(s) mismatched: {named}";
            ...
            catch (EngineCrashException ex)
            {
                ...
                if (fixture.Manifest.ExpectCrash)
                {
                    // A nonzero exit is not evidence of a genuine crash: a misconfigured adapter
                    // exits nonzero on every fixture. Credit the match only once this engine also
                    // completes an ordinary fixture from the same run.
                    MaterializedFixture control = fixtures.FirstOrDefault(candidate => !candidate.Manifest.ExpectCrash);
                    bool adapterIsFunctional = control == null || EngineCanRunControl(engine, control);
                    if (adapterIsFunctional)
                    {
                        result.Outcome = FixtureOutcome.Passed;
                    ...
                    else
                    {
                        result.Outcome = FixtureOutcome.Failed;
                        result.Reason =
                            $"engine crashed, but also failed the control fixture '{control.Id}' -- "
                            + $"treating as a harness-level problem, not a crash match: {ex.Message}";
                    }
                }
                else
                {
    ...
    /// <summary>
    /// Whether this exact engine can still complete an ordinary fixture, run fresh (not read back
    /// from an earlier result) so the check is independent of run order or a single-fixture run. Any
    /// exception -- crash, timeout, or otherwise -- means no: only a normal return counts.
    /// </summary>
    private static bool EngineCanRunControl(IEngine engine, MaterializedFixture control)
    {
        try
        {
            RunWithBudget(engine, control);
            return true;
        }
        catch
        {
