...
) : PostprocessBuildJob(platformService, engines, lockFactory, buildJobService, logger, sharedFileService)
{
    ...
    )
    {
        ...
        await TrainOnNewSegmentPairs(engineId, @lock, cancellationToken);

        await base.DoWorkAsync(engineId, buildId, data, buildOptions, @lock, cancellationToken);
    }
    ...
    private async Task DownloadBuiltEngineAsync(string engineId, CancellationToken cancellationToken)
    {
        ...
        await using FileStream fileStream = File.Create(localZipPath);
    ...
    private async Task TrainOnNewSegmentPairs(
        string engineId,
        IDistributedReaderWriterLock @lock,
        CancellationToken cancellationToken
    )
