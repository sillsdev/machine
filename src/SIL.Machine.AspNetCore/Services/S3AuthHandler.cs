...
/// </summary>
public class S3AuthHandler : DelegatingHandler
{
    ...
    private static readonly string EmptySha256 = ToHexString(SHA256(Array.Empty<byte>()));
    ...
    public S3AuthHandler(string accessKeyId, string secretAccessKey, string region, string service = "s3")
    ...
    protected async Task<string> SignAsync(HttpRequestMessage request, DateTimeOffset? signDate = null)
    {
        ...
        string signature = ToHexString(signatureRaw);

        string auth =
            $"Credential={_accessKeyId}/{nowDate}/{_region}/s3/aws4_request,SignedHeaders={signedHeaders},Signature={signature}";
    ...
    private static string GetCanonicalUri(HttpRequestMessage request)
    {
        ...
        return IOPath.Combine(ppts.Select(p => WebUtility.UrlEncode(p)));
    }

    private static string GetCanonicalQueryString(HttpRequestMessage request)
    {
        // CanonicalQueryString specifies the URI-encoded query string parameters. You URI-encode name and values
        // individually. You must also sort the parameters in the canonical query string alphabetically by key name. The
        // sorting occurs after encoding.
        NameValueCollection values = HttpUtility.ParseQueryString(request.RequestUri!.Query);
        var sb = new StringBuilder();

        // a. Sort the parameter names by character code point in ascending order. Parameters with duplicate names
        // should be sorted by value.For example, a parameter name that begins with the uppercase letter F precedes a
        ...
        foreach (string? key in values.AllKeys.OrderBy(k => k))
        {
            ...
            string? value = WebUtility.UrlEncode(values[key]);
            ...
            {
                sb.Append(WebUtility.UrlEncode(key)).Append('=').Append(value);
            }
        }
    ...
    private static string GetCanonicalHeaders(HttpRequestMessage request, out string signedHeaders)
    {
        ...
        // - Any x-amz-* headers that you plan to include in your request must also be added. For example, if you are
        // using temporary security credentials, you need to include x-amz-security-token in your request. You must add
        // this header in the list of CanonicalHeaders.
    ...
    private static async Task<string> AddPayloadHashHeader(HttpRequestMessage request)
    {
        ...
        if (request.Content != null)
        {
            ...
            hash = ToHexString(SHA256(content));
        }
        else
        {
            hash = EmptySha256;
    ...
    private static string ToHexString(byte[] bytes)
    {
        ...
        {
            byte b = bytes[i++];
            result[j++] = HexAlphabet[b >> 4];
            result[j++] = HexAlphabet[b & 0xF];
        }
    ...
    {
        return ToHexString(SHA256(Encoding.UTF8.GetBytes(s)));
    }
}
