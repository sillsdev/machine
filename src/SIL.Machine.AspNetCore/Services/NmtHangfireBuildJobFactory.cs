    ...
    public Job CreateJob(string engineId, string buildId, string stage, object? data)
    {
        return stage switch
        {
            NmtBuildStages.Preprocess
                => CreateJob<NmtPreprocessBuildJob, IReadOnlyList<Corpus>>(engineId, buildId, data),
            NmtBuildStages.Postprocess => CreateJob<NmtPostprocessBuildJob, (int, double)>(engineId, buildId, data),
            NmtBuildStages.Train => CreateJob<NmtTrainBuildJob>(engineId, buildId),
