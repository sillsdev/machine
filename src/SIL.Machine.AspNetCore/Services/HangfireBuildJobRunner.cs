...
{
    public static Job CreateJob<TJob, TData>(string engineId, string buildId, object? data)
        where TJob : HangfireBuildJob<TData>
    {
        if (data is null)
            throw new ArgumentNullException(nameof(data));
        // Token "None" is used here because hangfire injects the proper cancellation token
        return Job.FromExpression<TJob>(j => j.RunAsync(engineId, buildId, (TData)data, CancellationToken.None));
    }

    public static Job CreateJob<TJob>(string engineId, string buildId)
        where TJob : HangfireBuildJob
    {
        // Token "None" is used here because hangfire injects the proper cancellation token
        return Job.FromExpression<TJob>(j => j.RunAsync(engineId, buildId, CancellationToken.None));
    }
