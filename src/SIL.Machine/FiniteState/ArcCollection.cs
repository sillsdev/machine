...
{
    public class ArcCollection<TData, TOffset> : ICollection<Arc<TData, TOffset>>, IFreezable
        where TData : IAnnotatedData<TOffset>
    {
        ...
        public State<TData, TOffset> Add(FeatureStruct input, State<TData, TOffset> target)
        {
            ...
            return AddInternal(
                new Arc<TData, TOffset>(
                    _state,
                    new Input(input, 1),
                    new PriorityUnionOutput<TData, TOffset>(FeatureStruct.New().Value).ToEnumerable(),
                    target
                )
            );
        }
        ...
        public State<TData, TOffset> Add(
            FeatureStruct input,
        ...
        )
        {
            ...
            return AddInternal(
                new Arc<TData, TOffset>(_state, new Input(input, 1), outputAction.ToEnumerable(), target)
            );
        }

        internal State<TData, TOffset> Add(
            Input input,
