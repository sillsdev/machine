...
namespace SIL.Machine.Clusterers
{
    public class LsdbcClusterer<T> : IFlatClusterer<T>
    {
        ...
        public IEnumerable<Cluster<T>> GenerateClusters(IEnumerable<T> dataObjects)
        {
            ...
            {
                if (
                    unclassified.Contains(pointPair.Key)
                    && pointPair.Value.Item2.All(neighbor => pointPair.Value.Item1 <= pointInfos[neighbor].Item1)
                )
                {
        ...
        private void ExpandCluster(
            HashSet<T> unclassified,
            Dictionary<T, Tuple<double, List<T>>> pointInfos,
            HashSet<T> currentCluster,
            T point,
