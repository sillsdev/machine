...
namespace SIL.Machine.Tokenization
{
	public class ZwspWordDetokenizer : LatinWordDetokenizer
	{
		...
		protected override string GetSeparator(IReadOnlyList<string> tokens, IReadOnlyList<DetokenizeOperation> ops,
			int index)
		{
			if (index < tokens.Count - 1 && ops[index + 1] == DetokenizeOperation.MergeRight
