...
namespace SIL.Machine.Translation
{
	internal abstract class EditDistance<TSeq, TItem>
	{
		...
		protected double ComputeDistMatrix(TSeq x, TSeq y, bool isLastItemComplete, bool usePrefixDelOp,
			out double[,] distMatrix)
		{
			...
			for (int i = 0; i <= xCount; i++)
			{
				for (int j = 0; j <= yCount; j++)
				{
					int iPred, jPred;
					EditOperation op;
					distMatrix[i, j] = ProcessDistMatrixCell(x, y, distMatrix, usePrefixDelOp,
		...
		protected double ProcessDistMatrixCell(TSeq x, TSeq y, double[,] distMatrix, bool usePrefixDelOp,
			bool isComplete, int i, int j, out int iPred, out int jPred, out EditOperation op)
		{
			if (i != 0 && j != 0)
			{
		...
		protected IEnumerable<EditOperation> GetOperations(TSeq x, TSeq y, double[,] distMatrix, bool isLastItemComplete,
			bool usePrefixDelOp, int i, int j)
		{
			...
			{
				EditOperation op;
				ProcessDistMatrixCell(x, y, distMatrix, usePrefixDelOp, j != yCount || isLastItemComplete, i, j, out i,
					out j, out op);
