...
namespace SIL.Machine.Translation
{
	internal class SegmentEditDistance : EditDistance<string[], string>
	{
			...
			out IEnumerable<EditOperation> wordOps, out IEnumerable<EditOperation> charOps)
		{
			...
			{
				EditOperation op;
				ProcessDistMatrixCell(x, y, distMatrix, usePrefixDelOp, j != y.Length || isLastItemComplete, i, j,
					out i, out j, out op);
			...
			string[] yIncr, bool isLastItemComplete)
		{
			...
			for (int jIncr = 0; jIncr < yIncr.Length; jIncr++)
			{
				...
				int iPred, jPred;
				EditOperation op;
				double dist = ProcessDistMatrixCell(x, y, distMatrix, false, j != y.Length || isLastItemComplete,
