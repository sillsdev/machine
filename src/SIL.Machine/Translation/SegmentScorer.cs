...
namespace SIL.Machine.Translation
{
    public class SegmentScorer : IPairwiseAlignmentScorer<IReadOnlyList<string>, int>
    {
        ...
        public int GetExpansionScore(
            IReadOnlyList<string> sequence1,
            int p,
            IReadOnlyList<string> sequence2,
            int q1,
            int q2
        )
        {
            return (
                    GetSubstitutionScore(sequence1, p, sequence2, q1)
                    + GetSubstitutionScore(sequence1, p, sequence2, q2)
                ) / 2;
        }
        ...
        {
            return (
                    GetSubstitutionScore(sequence1, p1, sequence2, q)
                    + GetSubstitutionScore(sequence1, p2, sequence2, q)
                ) / 2;
        }
        ...
        {
            return (
                    GetSubstitutionScore(sequence1, p1, sequence2, q2)
                    + GetSubstitutionScore(sequence1, p2, sequence2, q1)
                ) / 2;
        }
