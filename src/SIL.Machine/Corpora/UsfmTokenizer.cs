	...
	public class UsfmTokenizer
	{
		...
		public IReadOnlyList<UsfmToken> Tokenize(string usfm, bool preserveWhitespace = false)
		{
			...
			while (index < usfm.Length)
			{
				...
				if (marker.StartsWith("+", StringComparison.Ordinal) && tag.StyleType != UsfmStyleType.Character
				...
				switch (tag.StyleType)
				{
					case UsfmStyleType.Character:
						...
						{
							tokens.Add(new UsfmToken(UsfmTokenType.Verse, marker, null, null,
					...
					case UsfmStyleType.Paragraph:
						...
						if ((tag.TextProperties & UsfmTextProperties.Chapter) > 0)
							tokens.Add(new UsfmToken(UsfmTokenType.Chapter, marker, null, null,
								GetNextWord(usfm, ref index, preserveWhitespace)));
						else if ((tag.TextProperties & UsfmTextProperties.Book) > 0)
							tokens.Add(new UsfmToken(UsfmTokenType.Book, marker, null, null,
								GetNextWord(usfm, ref index, preserveWhitespace)));
						else
							tokens.Add(new UsfmToken(UsfmTokenType.Paragraph, marker, null, endMarker));
						break;
					case UsfmStyleType.Note:
						tokens.Add(new UsfmToken(UsfmTokenType.Note, marker, null, endMarker,
							GetNextWord(usfm, ref index, preserveWhitespace)));
						break;
			...
			int nextMarkerIndex, ref string text)
		{
			...
			if (matchingToken.SetAttributes(attributesValue, matchingTag.DefaultAttributeName, ref adjustedText,
				preserveWhitespace))
			{
				text = adjustedText;
				...
				{
					attributeToken = new UsfmToken(UsfmTokenType.Attribute, matchingTag.Marker, null, null,
						attributesValue);
