...
{
	public class ParallelPermutationRuleCascade<TData, TOffset> : PermutationRuleCascade<TData, TOffset> where TData : IAnnotatedData<TOffset>
	{
		...
		public ParallelPermutationRuleCascade(IEnumerable<IRule<TData, TOffset>> rules, IEqualityComparer<TData> comparer)
			: base(rules, comparer)
		{
		}
		...
		public ParallelPermutationRuleCascade(IEnumerable<IRule<TData, TOffset>> rules, bool multiApp, IEqualityComparer<TData> comparer)
				...
				Parallel.ForEach(from, work =>
					{
						for (int i = work.Item2; i < Rules.Count; i++)
						{
							...
							if (results.Length > 0)
							{
								...
								Tuple<TData, int>[] workItems = results.Where(res => !MultipleApplication || !Comparer.Equals(work.Item1, res))
									.Select(res => Tuple.Create(res, MultipleApplication ? i : i + 1)).ToArray();
