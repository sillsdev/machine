...
namespace SIL.Machine.Threading
{
	...
	{
		public static async Task<T> Timeout<T>(Func<CancellationToken, Task<T>> action, TimeSpan timeout,
		...
		public static async Task Timeout(Func<CancellationToken, Task> action, TimeSpan timeout,
			CancellationToken cancellationToken = default)
		{
			if (timeout == System.Threading.Timeout.InfiniteTimeSpan)
			{
