...
namespace SIL.Machine.Morphology
{
    ...
    public class WordAnalysis : IEquatable<WordAnalysis>
    {
        ...
        {
            return other != null
                && Morphemes.Select(m => m.Id).SequenceEqual(other.Morphemes.Select(m => m.Id))
                && RootMorphemeIndex == other.RootMorphemeIndex
        ...
        public override string ToString()
        {
            return string.Format(
                "[{0}]",
                string.Join(" ", Morphemes.Select((m, i) => i == RootMorphemeIndex ? "*" + m : m.ToString()))
            );
        }
    }
}
