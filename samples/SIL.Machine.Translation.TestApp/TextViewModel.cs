...
namespace SIL.Machine.Translation.TestApp
{
    public class TextViewModel : ViewModelBase, IChangeTracking
    {
        ...
        public TextViewModel(
            ...
            HybridTranslationEngine engine,
            string name,
            ...
            using (var textWriter = new StreamWriter(_targetFileName))
            {
                for (int i = 0; i < _targetSegments.Count; i++)
                {
                    metadataWriter.WriteLine(
                        "{0}\t{1}",
                        _paragraphs.Contains(i) || i == 0 ? "1" : "0",
            ...
            set
            {
                if (_isActive != value)
                {
                    ...
                    if (_isActive)
                    {
                        ...
                        {
                            int unapprovedSegment = Enumerable
                                .Range(0, _sourceSegments.Count)
        ...
        private (int, int)[] GetAlignedWords(WordAlignmentMatrix matrix)
        {
            return Enumerable
                ...
                .Where(t => matrix[t.SourceIndex, t.TargetIndex])
                .ToArray();
        }
        ...
        {
            int segmentIndex = _sourceSegments.IndexOf(
                s => s.StartIndex <= index && s.StartIndex + s.Text.Length > index
            );
            if (segmentIndex != -1)
            {
        ...
        {
            int segmentIndex = _targetSegments.IndexOf(
                s => s.StartIndex <= index && s.StartIndex + s.Text.Length > index
            );
            if (segmentIndex != -1)
                MoveSegment(segmentIndex);
        ...
        private void MoveSegment(int segmentIndex)
        {
            ...
            CurrentSourceSegmentRange = new Range<int>(
                sourceSegment.StartIndex,
                sourceSegment.StartIndex + sourceSegment.Text.Length
            );
            CurrentTargetSegmentRange = new Range<int>(
        ...
        private void UpdateSuggestions()
        {
            ...
            {
                _suggestions.ReplaceAll(
                    _suggester
                        .GetSuggestions(_translator)
                        .First()
        ...
        private void UpdateSourceSegmentSelection()
        {
            ...
            int targetWordIndex = _tokenizer
                .TokenizeAsRanges(TargetSegment)
            ...
            if (targetWordIndex != -1)
            {
                ...
                if (confidence >= _suggester.ConfidenceThreshold || (sources & TranslationSources.Transfer) != 0)
                {
                    foreach (int sourceIndex in result.Alignment.GetColumnAlignedIndices(targetWordIndex))
                    {
                        ...
                        Annotations.Range<int> range = _tokenizer
                            .TokenizeAsRanges(SourceSegment)
        ...
        {
            _unapprovedTargetSegmentRanges.ReplaceAll(
                _targetSegments
