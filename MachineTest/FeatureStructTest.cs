...
namespace SIL.Machine.Test
{
	...
	public class FeatureStructTest
	{
		...
		public void Unify()
		{
			...
			TestBinaryOperation(ValueEqualityComparer<FeatureStruct>.Default, resultsSelector, varResultsSelector,
				// simple
				featSys => null,
			...
			Assert.That(result, Is.EqualTo(FeatureStruct.New(featureSystem)
				...
				.Feature("i").ReferringTo(1).Value).Using((IEqualityComparer<FeatureStruct>) ValueEqualityComparer<FeatureStruct>.Default));
		}
