...
{
	public delegate Annotation<TOffset> ApplyDelegate<TData, TOffset>(TData input, PatternMatch<TOffset> match, out TData output) where TData : IData<TOffset>; 
	...
	public class DelegatePatternRuleAction<TData, TOffset> : IPatternRuleAction<TData, TOffset> where TData : IData<TOffset>
	{
		...
		{
			return _action(input, match, out output);
		}
	}
}
