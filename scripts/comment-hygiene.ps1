...
function Invoke-SelfTest {
	...
	Test-Case 'powershell help block is budget exempt' 'help.ps1' @(
		'<#', '.SYNOPSIS', "	$sixty", "	$sixty", "	$sixty", "	$sixty", '#>', '$a = 1'
	) @()
	Test-Case 'content rules still apply inside a help block' 'helpbad.ps1' @(
		'<#', '.DESCRIPTION', "	Extracted from the old tokenizer.", '#>', '$a = 1'
	) @('provenance')
	Test-Case 'powershell comment uses the same budget' 'budget.ps1' @(
		"# $sixty", "# $sixty", "# $sixty", "# $sixty", '$a = 1'
	) @('comment-too-long')
	Test-Case 'a one-line help block ends a block' 'split.ps1' @(
		"# $sixty", "# $sixty", '<# Reads one line. #>', "# $sixty", "# $sixty", '$a = 1'
	) @()
	Test-Case 'a module file is scanned like a script' 'rules.psm1' @(
		'# This helper is no longer used by the loader.', '$a = 1'
	) @('absence-narration')
